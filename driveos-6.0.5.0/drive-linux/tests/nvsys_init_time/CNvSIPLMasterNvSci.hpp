 ...
 public:
    ...
    SIPLStatus SetupSync(bool bNeedCpuWaiter)
    {
        ...
        for (ProducerStream *pStream : m_usedStreams) {
            ...
            if (bNeedCpuWaiter) {
                // Only set CPU access attribute here since CPU waiter attribute is set elsewhere
                sciErr = NvSciSyncAttrListSetAttrs(pStream->signalerAttrList, keyVals, 1);
                CHK_NVSCISTATUS_AND_RETURN(sciErr, "Producer signaler NvSciSyncAttrListSetAttrs for file write mode");
            }
       ...
       for (ProducerStream *pStream : m_usedStreams) {
          NvSciSyncAttrList unreconciledList[MAX_OUTPUTS_PER_SENSOR];
