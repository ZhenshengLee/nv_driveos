...
#include "platform/ar0820_single.hpp"
...
public:
    ...
    SIPLStatus Init(uint32_t uSensor, INvSIPLNotificationQueue *notificationQueue, std::vector<CProfiler*> vpProfilerISP)
    {
        if (notificationQueue == nullptr) {
            LOG_ERR("Invalid Notification Queue\n");
            return NVSIPL_STATUS_BAD_ARGUMENT;
...
public:
    ...
    static void FrameCompletionQueueThreadFunc(CPipelineFrameQueueHandler *pThis)
    {
        ...
        while (!pThis->m_bQuit) {
            ...
            counter++;
            if (counter == numDev && !(pThis->firstFrameComplete)) {
                recordTimestampInCarveout("nvsys_init_time: Got first camera frames");
                pThis->firstFrameComplete = true;
            }
        }
    }
...
static void suspend_drive_os(void)
{
    ...
    //TODO: Invoke DriveOsSuspend(). For now, sleep for
    //      a short duration.
    sleep(5);
...
int main(int argc, char *argv[])
{
    ...
    if (cmdline.sConfigName.compare(platformCfgAr0820.platformConfig) == 0) {
        printf("Skipping Query, using hardcoded platform config\n");
        oPlatformCfg = platformCfgAr0820;
    }
    else if (cmdline.sConfigName.compare(platformCfgAr0820_s.platformConfig) == 0) {
        printf("Skipping Query, using hardcoded platform config\n");
        oPlatformCfg = platformCfgAr0820_s;
    }
    else {
        // INvSIPLQuery
    ...
    numDev = (int)oPlatformCfg.numDeviceBlocks;
    printf("numDev is %d\n", numDev);
...
#else
        auto status = clockIdMgr.GetClockId((NvSiplTimeBase)i, clockId);
        if (status != NVSIPL_STATUS_OK) {
            LOG_ERR("CClockIdMgr::GetClockId failed\n");
            return status;
        }
    ...
    auto status = upMaster->Setup(&bufModule, &syncModule);
    ...
    if ((upComposite != nullptr) && cmdline.bNvSci) {
        ...
        //nvsipl_camera passes whether profiling is enabled here. This app has it enabled by default so pass true
        status = upMaster->SetupSync(true);
        if (status != NVSIPL_STATUS_OK) {
