...
)
{
    ...
    else
    {
        ...
        LOG_INFO("ProcessProfileData: avg sw time %f ms, avg hw time %f ms, avg sync wait time %f ms \n",
                 avgSwTimeInMS,
                 avgHwTimeInMS,
                 avgSyncWaitTimeInMS);
    }
...
**/
    ...
    if (args.profile)
    {
        ctx.profDataArray = malloc(sizeof(NvMediaIofaProfileData)*args.numFrames);
    }
