...
)
{
    ...
    for (uint8_t i = 0; i < countNvSciBufObj; i++)
    {
      ...
      if (taskStatus.status != NvSciSyncTaskStatus_Success) {
        LOG_ERR("TaskStatus shows %d \n", taskStatus.status);
      }
      if (taskStatus.status == NvSciSyncTaskStatus_Failure) {
...
int main(int argc, char *argv[])
{
    ...
    status = GetInputFormatAttributes(args.inputFileFormat,
            &inputFileChromaFormat, &uvOrderFlag, &pixelAlignment);
    if (NVMEDIA_STATUS_OK != status) {
    ...
    status = MapInputFileChromaToSurfaceChroma(
            inputFileChromaFormat, &surfaceChromaFormat);
    if (NVMEDIA_STATUS_OK != status) {
    ...
    if (args.profileEnable) {
        ...
        NvpRecordSample(&initLatency, initStartTimeMark, initStopTimeMark);
    }
#endif
    ...
    // Read the input images in advance in case of perf test
    if (args.perfTest
#if ENABLE_PROFILING
        || args.profileEnable
#endif
...
Done:
    ...
    if (args.perfTest) {
        LOG_MSG("\nTotal Encoding time for %d frames: %.3f ms\n", totalFramesEncoded, elapse);
        LOG_MSG("Coding time per frame %.4f ms \n", elapse / totalFramesEncoded);
        LOG_MSG("encode fps achieved %.4f \n", 1000.0 * totalFramesEncoded / elapse);
        if (args.avgTimePerFrame < (elapse / totalFramesEncoded)) {
            LOG_ERR("main: perf test failure");
            goto fail;
        }
    }
#if ENABLE_PROFILING
    ...
    {
        NvpStatus_t status;
        NvpPrintStats(&initLatency, NULL, USEC, "Initialization Latency", false);
        status = NvpDumpData(&initLatency);
        if (status != NVP_PASS)
        {
        ...
        NvpPrintStats(&encOutThreadArgs.latencies, NULL, USEC, "Execution Latency", false);
        status = NvpDumpData(&encOutThreadArgs.latencies);
        if (status != NVP_PASS)
        {
            LOG_ERR("main: NvpDumpData failed with Error %d\n", status);
        }
        NvpDestroyPerfData(&initLatency);
