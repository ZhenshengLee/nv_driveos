...
void PrintUsage()
{
    ...
    LOG_MSG("-alternateCreateAPI                Use alternate create API of IEP element \n");
...
int ParseArgs(int argc, char **argv, TestArgs *args)
{
    ...
    ConfigParamsMap paramsMap[] = {
        ...
        {"H264EnableNewPreset",                &args->configH264Params.enableNewPreset,                TYPE_UCHAR,    0, LIMITS_BOTH, 0, 1,   0, 0, SECTION_NONE},
        ...
        {"H265EnableNewPreset",                &args->configH265Params.enableNewPreset,                TYPE_UCHAR,    0, LIMITS_BOTH, 0, 1,   0, 0, SECTION_NONE},
	...
	// NvMediaEncodeConfigVP9
        ...
        {"AV1EncodeQuality",                  &args->configAV1Params.quality,                          TYPE_UINT,     1, LIMITS_BOTH, 1, 2,   0, 0, SECTION_NONE},
        {"AV1EncPreset",                      &args->configAV1Params.encPreset,                        TYPE_UINT_HEX, 0x10, LIMITS_BOTH, 0x0, 0x20,   0, 0, SECTION_NONE},
        {"AV1EnableNewPreset",                &args->configAV1Params.enableNewPreset,                  TYPE_UCHAR,    0, LIMITS_BOTH, 0, 1,   0, 0, SECTION_NONE},
    ...
    while(paramsCounter < argc) {
        if(strcasecmp(argv[paramsCounter], "-h") == 0) {
            ...
            paramsCounter++;
        } else if(strcasecmp(argv[paramsCounter], "-pt") == 0) {
            // perf test
            args->perfTest = true;
            if(argv[paramsCounter + 1] && argv[paramsCounter + 1][0] != '-') {
                args->avgTimePerFrame = atof(argv[paramsCounter + 1]);
                paramsCounter++;
            } else {
                LOG_ERR("ParseArgs: -pt must be followed by time in seconds\n");
                return -1;
            }
            paramsCounter++;
#if ENABLE_PROFILING
