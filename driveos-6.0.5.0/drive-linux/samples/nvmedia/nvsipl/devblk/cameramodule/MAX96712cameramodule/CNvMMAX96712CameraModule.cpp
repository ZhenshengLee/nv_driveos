...
CNvMMAX96712CameraModule::Init()
{
    ...
    m_broadcastSensor = std::move(CreateBroadcastSensor(m_linkIndex));

    status = m_pDeserializer->EnableLinks(1 << m_linkIndex);
    if (status != NVSIPL_STATUS_OK) {
        SIPL_LOG_ERR_STR_INT("MAX96712: Enable all transport links failed with SIPL error", (int32_t)status);
        return status;
    }
    ...
    if (m_groupInitProg) {
        ...
        if (((m_initLinkMask & (0xF << m_linkIndex)) & 0xF) == (1 << m_linkIndex)) {
            ...
            if (status != NVSIPL_STATUS_OK) {
                SIPL_LOG_ERR_STR_INT("MAX96712: Check Link Lock failed with SIPL error", (int32_t)status);
                return status;
            }
    ...
    if (status != NVSIPL_STATUS_OK) {
        SIPL_LOG_ERR_STR_INT("MAX96712: Broadcast Sensor Deinit failed with SIPL error", (int32_t)status);
        return status;
    }
...
CNvMMAX96712CameraModule::Reconfigure()
{
    ...
    if (status != NVSIPL_STATUS_OK) {
        ...
        item.eAction = CNvMDeserializer::LinkAction::LINK_DISABLE;
        linkAction.clear();
...
CNvMMAX96712CameraModule::CreateBroadcastSensor(const uint8_t linkIndex)
{
    CNvMDevice::DeviceParams params = m_oDeviceParams;
    params.bUseNativeI2C = true;
    ...
    DevBlkCDIRootDevice* cdiRoot, const uint8_t linkIndex)
{
    ...
    std::unique_ptr<CNvMSensor> broadcastSensor = std::move(CreateBroadcastSensor(m_linkIndex));
    ...
    if (!m_oDeviceParams.bPassive && !m_oDeviceParams.bEnableSimulator) {
        ...
        if ((m_camPwrControlInfo.method) && (m_camPwrControlInfo.method != UINT8_MAX)) {
            ...
            status = m_upCampwr->InitPowerDevice(m_upCampwr->GetDeviceHandle(), m_camPwrControlInfo.links[linkIndex]);
            if (status != NVSIPL_STATUS_OK) {
...
CNvMCameraModule_SetDebugLevel(
    INvSIPLDeviceBlockTrace::TraceLevel level)
{
#if !NV_IS_SAFETY
