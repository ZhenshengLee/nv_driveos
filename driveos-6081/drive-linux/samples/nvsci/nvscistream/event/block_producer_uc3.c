    ...
    ProdData*         prodData)
{
    ...
    crcBufCount = prodData->numConsumers + 1;
    tmpBuf = (uint32_t*)calloc(crcBufCount, sizeof(uint32_t));
    ...
    ProdData* prodData)
{
    ...
    /* Validate the CRC value with that from pool */
    if (poolCrcData->buf != prodData->initCrc.buf[prodData->numConsumers]) {
