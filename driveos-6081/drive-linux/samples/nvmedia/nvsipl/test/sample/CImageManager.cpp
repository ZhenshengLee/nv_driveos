...
SIPLStatus CImageManager::AllocateBuffers(ImagePool &imagePool)
{
    ...
    std::unique_ptr<NvSciBufAttrList, CloseNvSciBufAttrList> ur2dAttrList;
    ...
    size_t numAttrLists = 1u;
    NvSciBufAttrList unreconciledAttrLists[2];
    unreconciledAttrLists[0] = *imagePool.attrList;

#ifdef NVMEDIA_QNX
    if (m_disp) {
        ur2dAttrList.reset(new NvSciBufAttrList());
        err = NvSciBufAttrListCreate(m_sciBufModule, ur2dAttrList.get());
        CHK_NVSCISTATUS_AND_RETURN(err, "NvSciBufAttrListCreate");
        NvSciBufType bufType = NvSciBufType_Image;
        NvSciBufAttrKeyValuePair attrKvp = {
            NvSciBufGeneralAttrKey_Types, &bufType, sizeof(bufType),
        };
        err = NvSciBufAttrListSetAttrs(*ur2dAttrList, &attrKvp, 1U);
        CHK_NVSCISTATUS_AND_RETURN(err, "NvSciBufAttrListSetAttrs");

        NvMediaStatus nvmStatus = NvMedia2DFillNvSciBufAttrList(nullptr, *ur2dAttrList);
        CHK_NVMSTATUS_AND_RETURN(nvmStatus, "NvMedia2DFillNvSciBufAttrList");

        unreconciledAttrLists[1] = *ur2dAttrList;
        numAttrLists++;
    }
#endif // NVMEDIA_QNX

    err = NvSciBufAttrListReconcile(unreconciledAttrLists,
                                    numAttrLists,
