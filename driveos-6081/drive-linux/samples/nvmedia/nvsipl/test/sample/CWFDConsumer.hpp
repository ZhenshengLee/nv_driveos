...
public:
    ...
    SIPLStatus Init(NvSciBufModule &bufModule,
                    ...
                    uint32_t &uWidth,
                    uint32_t &uHeight);
    void GetBuffers(std::vector<NvSciBufObj>& vBuffers);
    SIPLStatus WfdFlip(NvSciBufObj& bufObj);
    SIPLStatus NullFlip();
...
private:
    ...
    SIPLStatus InitWFD(uint32_t &uWidth,
                       uint32_t &uHeight);
    SIPLStatus RegisterBuffers(uint32_t uWidth,
                               uint32_t uHeight);
    ...
    std::vector<std::pair<NvSciBufObj, WFDSource>> m_vBufSourcePair;
    CNvSciBufResources m_nvSciBufRes;
    std::vector<NvSciBufObj> m_vBufObj;
