...
SIPLStatus CUtils::IsRawBuffer(NvSciBufObj bufObj, bool &bIsRaw)
{
    ...
    if (((colorFmt >= NvSciColor_Bayer8RGGB) && (colorFmt <= NvSciColor_Signed_X12Bayer20GBRG)) ||
         ((colorFmt >= NvSciColor_X6Bayer10BGGI_RGGI) && (colorFmt <= NvSciColor_X4Bayer12RGGB_RJ))) {
        bIsRaw = true;
    }
    return NVSIPL_STATUS_OK;
}

SIPLStatus CUtils::IsRGBIRBuffer(NvSciBufObj bufObj, bool &bIsRGBIR)
{
    bIsRGBIR = false;
    BufferAttrs bufAttrs;
    SIPLStatus status = PopulateBufAttr(bufObj, bufAttrs);
    CHK_STATUS_AND_RETURN(status, "PopulateBufAttr");
    NvSciBufAttrValColorFmt colorFmt = bufAttrs.planeColorFormats[0];
    if ((colorFmt >= NvSciColor_X6Bayer10BGGI_RGGI) && (colorFmt <= NvSciColor_Bayer16IGGR_IGGB)) {
        bIsRGBIR = true;
    }
    return NVSIPL_STATUS_OK;
}

SIPLStatus CUtils::CreateScratchBuffer(NvSciBufModule &bufModule,
                                       ...
                                       bool isRGBIR)
{
    ...
    if (isRGBIR) {
        planeColorFmt = NvSciColor_Y8;
        planeColorStd = NvSciColorStd_REC709_ER;
    }
...
SIPLStatus CUtils::ConvertRaw(NvSciBufObj srcBufObj,
                              uint8_t *pSrcBuf,
                              ...
                              uint8_t *pDstBuf)
{
    ...
    uint32_t xR = 0U, yR = 0U, xG1 = 0U, yG1 = 0U, xG2 = 0U, yG2 = 0U, xB = 0U, yB = 0U, xI1 = 0U, yI1 = 0U;
    switch (srcBufAttrs.planeColorFormats[0]) {
        ...
        case NvSciColor_Signed_X12Bayer20CCCC:
            ...
            xR = 1U; yR = 1U;
            break;
        case NvSciColor_X6Bayer10BGGI_RGGI:
        case NvSciColor_X6Bayer10RGGI_BGGI:
            isRGBIR = true;
            xG1 = 1U; yG1 = 0U;
            xI1 = 1U; yI1 = 1U;
            break;
    ...
    for (uint32_t y = srcBufAttrs.topPadding[0];
        ...
        for (uint32_t x = 0U; x < srcBufAttrs.planeWidths[0]; x += 2U) {
            if (isRGBIR) {
                // Y (average of G1 and I1)
                uint32_t g1 = pSrcBuf[srcPitch*(y + yG1) + 2U*(x + xG1) + 1U];
                uint32_t i1 = pSrcBuf[srcPitch*(y + yI1) + 2U*(x + xI1) + 1U];
                *pDstBuf++ = (g1 + i1)/2U;
                // Skip R, B and the other G component
            } else {
                if (pixIsRJ) {
                    // R
...
SIPLStatus PopulateBufAttr(const NvSciBufObj& sciBufObj, BufferAttrs &bufAttrs)
{
    ...
    if (imgAttrs[1].len != 0) {
        ...
        LOG_WARN("Skipped populating layout as length of attribute was 0\n");
    }
    ...
    if (imgAttrs[14].len != 0) {
        ...
        LOG_WARN("Skipped populating needSwCacheCoherency as length of attribute was 0\n");
    }
    ...
    if (imgAttrs[3].len != 0) {
        ...
        LOG_WARN("Skipped populating planeWidths as length of attribute was 0");
    }
    ...
    if (imgAttrs[4].len != 0) {
        ...
        LOG_WARN("Skipped populating planeHeights as length of attribute was 0\n");
    }
    ...
    if (imgAttrs[5].len != 0) {
        ...
        LOG_WARN("Skipped populating planePitches as length of attribute was 0\n");
    }
    ...
    if (imgAttrs[6].len != 0) {
        ...
        LOG_WARN("Skipped populating planeBitsPerPixels as length of attribute was 0\n");
    }
    ...
    if (imgAttrs[7].len != 0) {
        ...
        LOG_WARN("Skipped populating planeAlignedHeights as length of attribute was 0\n");
    }
    ...
    if (imgAttrs[8].len != 0) {
        ...
        LOG_WARN("Skipped populating planeAlignedSizes as length of attribute was 0\n");
    }
    ...
    if (imgAttrs[9].len != 0) {
        ...
        LOG_WARN("Skipped populating planeChannelCounts as length of attribute was 0\n");
    }
    ...
    if (imgAttrs[10].len != 0) {
        ...
        LOG_WARN("Skipped populating planeOffsets as length of attribute was 0\n");
    }

    if (imgAttrs[11].len != 0) {
        memcpy(bufAttrs.planeColorFormats,
               static_cast<const NvSciBufAttrValColorFmt*>(imgAttrs[11].value),
               bufAttrs.planeCount * sizeof(bufAttrs.planeColorFormats[0]));
    } else {
        LOG_WARN("Skipped populating planeColorFormats as length of attribute was 0\n");
    }

    if (imgAttrs[12].len != 0) {
        memcpy(bufAttrs.topPadding,
               static_cast<const uint64_t*>(imgAttrs[12].value),
               bufAttrs.planeCount * sizeof(bufAttrs.topPadding[0]));
    } else {
        LOG_WARN("Skipped populating topPadding as length of attribute was 0\n");
    }

    if (imgAttrs[13].len != 0) {
        memcpy(bufAttrs.bottomPadding,
               static_cast<const uint64_t*>(imgAttrs[13].value),
               bufAttrs.planeCount * sizeof(bufAttrs.bottomPadding[0]));
    } else {
        LOG_WARN("Skipped populating bottomPadding as length of attribute was 0\n");
    }
