 ...
 public:
                                 ...
                                 NvSciSyncCpuWaitContext cpuWaitContext)
    {
        ...
        if (isPackedYUV && m_isRawOutput)
        {
            ...
            uint32_t pixelStartHeight = bufAttrs.topPadding[0];
            uint32_t pixelEndHeight = bufAttrs.planeHeights[0] - bufAttrs.bottomPadding[0];
            /* Note that if users are interested in the embedded data also being
             * written out to a file they would need to set the above values
             * to 0 and planeHeights[0]
             */
            uint8_t* basePtr = static_cast<uint8_t*>(va_ptr);
            for (uint32_t j = pixelStartHeight; j < pixelEndHeight; j++){
