...
#include "AR0820NonFuSaCustomInterface.hpp" // Custom interface for AR0820, sample
#include "IMX728_CustomInterface.hpp" // Custom interface for IMX728, sample
#include "IMX623_CustomInterface.hpp" // Custom interface for IMX623, sample
#include "MAX96712_Fusa_nv_CustomInterface.hpp" // Custom interface for MAX96712_Fusa_nv deserializer, sample
...
class CNvSIPLMaster
{
 protected:
    ...
    Interface *m_customInterface[MAX_SENSORS] = {NULL};
 ...
 public:
        ...
        for (INvSIPLClient::ConsumerDesc::OutputType outputType : outputList) {
            ...
            for (uint32_t i = 0U; i < NUM_SYNC_INTERFACES; i++) {
                if ((syncObjTypeSipl[i] == NVSIPL_EOFSYNCOBJ)
                    ...
                    // We need CPU attributes both SIPL EOF Fence Obj and 2D EOF Fence Obj
                    // SIPL EOF Fence Obj - For profiling
                    // 2D EOF Fence Obj - CPU waits before pushing output buffer to display
    ...
    SIPLStatus SetTwoPassIspSync(uint32_t producerIdx,
                                 uint32_t consumerIdx,
                                 bool isp0Enabled,
                                 bool isp1Enabled,
                                 bool isp2Enabled)
    {
        ...
        INvSIPLClient::ConsumerDesc::OutputType ispOutputType;
        if (isp0Enabled) {
            ispOutputType = INvSIPLClient::ConsumerDesc::OutputType::ISP0;
        } else if (isp1Enabled) {
            ispOutputType = INvSIPLClient::ConsumerDesc::OutputType::ISP1;
        } else if (isp2Enabled) {
            ispOutputType = INvSIPLClient::ConsumerDesc::OutputType::ISP2;
        } else {
            LOG_ERR("At least 1 ISP needs to be enabled for Two Pass ISP mode\n");
    ...
    SIPLStatus AllocateCPUWaiter(uint32_t pip, bool isp0Enabled, bool isp1Enabled, bool isp2Enabled)
    {
        INvSIPLClient::ConsumerDesc::OutputType ispOutputType;
        if (isp0Enabled) {
            ispOutputType = INvSIPLClient::ConsumerDesc::OutputType::ISP0;
            ...
            LOG_WARN("Skipping CPU waiter setup since no ISP output was enabled\n");
            return NVSIPL_STATUS_OK;
        }
        ...
        sciErr = NvSciSyncObjAlloc(reconciledList, syncObj.get());
        CHK_NVSCISTATUS_AND_RETURN(sciErr, "Reconciled signaler and waiter NvSciSyncObjAlloc");

        m_sciSyncObjs[pip][(uint32_t)ispOutputType] = std::move(syncObj);
    ...
    SIPLStatus RegisterCPUWaiter(uint32_t pip, bool isp0Enabled, bool isp1Enabled, bool isp2Enabled)
    {
        INvSIPLClient::ConsumerDesc::OutputType ispOutputType;
        if (isp0Enabled) {
            ispOutputType = INvSIPLClient::ConsumerDesc::OutputType::ISP0;
        ...
        auto status = m_pCamera->RegisterNvSciSyncObj(pip,
                                                      ispOutputType,
                                                      NVSIPL_EOFSYNCOBJ,
                                                      *m_sciSyncObjs[pip][(uint32_t)ispOutputType]);
        CHK_STATUS_AND_RETURN(status, "INvSIPLCamera::RegisterNvSciSyncObj");

        m_sciSyncObjs[pip][(uint32_t)ispOutputType].reset();
    ...
    virtual SIPLStatus InitCustomInterfaces()
    {
        ...
        for (auto d = 0u; d != m_platformCfg.numDeviceBlocks; d++) {
            ...
            for (auto m = 0u; m != db.numCameraModules; m++) {
                ...
                SIPLStatus status = m_pCamera->GetModuleInterfaceProvider(uSensor, moduleInterfaceProvider);
                if (status != NVSIPL_STATUS_OK) {
                    ...
                    else if(m_platformCfg.deviceBlockList[d].cameraModuleInfoList[m].sensorInfo.name.compare("IMX728") == 0)
                        uuid = IMX728_CUSTOM_INTERFACE_ID;
                    else if(m_platformCfg.deviceBlockList[d].cameraModuleInfoList[m].sensorInfo.name.compare("IMX623") == 0)
                        uuid = IMX623_CUSTOM_INTERFACE_ID;
    ...
    SIPLStatus ShowEEPROM(uint32_t pip, bool write = false)
    {
        SIPLStatus status = NVSIPL_STATUS_OK;
        uint16_t addr = 0x4A;
        uint32_t length = 4;
...
#if !NV_IS_SAFETY
        if ((m_customInterface[pip] != NULL) && write)
        {
            addr = 0x1A0; /* Move to writable address space */
            ...
            uint32_t devBlkIndex;
            status = GetModuleAndDevBlkIndex(pip, moduleIndex, devBlkIndex);
            if (status != NVSIPL_STATUS_OK)
            {
                LOG_ERR("No module and devblk index found\n");
                return status;
            }

            if (m_platformCfg.deviceBlockList[devBlkIndex].cameraModuleInfoList[moduleIndex].sensorInfo.name.compare("IMX728") == 0) {
                IMX728_CustomInterface* imx728Interface =
                    static_cast<IMX728_CustomInterface*>(m_customInterface[pip]);
                status = imx728Interface->WriteEEPROMData(addr, length, buf);
                if ((status != NVSIPL_STATUS_OK) && (status != NVSIPL_STATUS_NOT_SUPPORTED))
                {
                    LOG_ERR("Failed to write EEPROM data via IMX728 custom SIPL API\n");
                    return status;
                } else if (status == NVSIPL_STATUS_OK) {
                    LOG_MSG("Wrote 0x%X 0x%X 0x%X 0x%X to EEPROM at 0x%X\n",
                            buf[0], buf[1], buf[2], buf[3], addr);
                }
            } else if (m_platformCfg.deviceBlockList[devBlkIndex].cameraModuleInfoList[moduleIndex].sensorInfo.name.compare("IMX623") == 0) {
                IMX623_CustomInterface* imx623Interface =
                    static_cast<IMX623_CustomInterface*>(m_customInterface[pip]);
                status = imx623Interface->WriteEEPROMData(addr, length, buf);
                if ((status != NVSIPL_STATUS_OK) && (status != NVSIPL_STATUS_NOT_SUPPORTED))
                {
                    LOG_ERR("Failed to write EEPROM data via IMX623 custom SIPL API\n");
                    return status;
                } else if (status == NVSIPL_STATUS_OK) {
                    LOG_MSG("Wrote 0x%X 0x%X 0x%X 0x%X to EEPROM at 0x%X\n",
                            buf[0], buf[1], buf[2], buf[3], addr);
        ...
        LOG_MSG("Reading %u byte(s) from EEPROM at 0x%X\n", length, addr);
        ...
        LOG_MSG("Read  0x%X 0x%X 0x%X 0x%X from EEPROM\n", buf[0], buf[1], buf[2], buf[3]);
    ...
    SIPLStatus CaptureFillAttributesAllocateSciBuf(uint32_t pip)
    {
        ...
        memset(&unreconciledAttrlist[0], 0, sizeof(unreconciledAttrlist));

        NvSciError err = NvSciBufAttrListCreate(m_sciBufModule, &unreconciledAttrlist[index]);
        ...
        err = NvSciBufAttrListSetAttrs(unreconciledAttrlist[index], attrKvp, 2);
        CHK_NVSCISTATUS_AND_RETURN(err, "NvSciBufAttrListSetAttrs");
        auto status = m_pCamera->GetImageAttributes(pip,
                                                    INvSIPLClient::ConsumerDesc::OutputType::ICP,
                                                    unreconciledAttrlist[index]);
        if (status != NVSIPL_STATUS_OK) {
...
#if !(NV_IS_SAFETY)
        if (m_pComposite != nullptr) {
            err = NvSciBufAttrListCreate(m_sciBufModule, &unreconciledAttrlist[index]);
            CHK_NVSCISTATUS_AND_RETURN(err, "NvSciBufAttrListCreate");

            NvSciBufAttrKeyValuePair compositeAttrKvp[] = {
                { NvSciBufGeneralAttrKey_Types, &bufType, sizeof(bufType) },
            };
            err = NvSciBufAttrListSetAttrs(unreconciledAttrlist[index], compositeAttrKvp, 1);
            CHK_NVSCISTATUS_AND_RETURN(err, "NvSciBufAttrListSetAttrs");

            status = m_pComposite->FillNvSciBufAttrList(unreconciledAttrlist[index]);
            if (status != NVSIPL_STATUS_OK) {
                LOG_ERR("Compositor FillNvSciBufAttrList failed\n");
                return status;
            }
            index++;
        }
        ...
        NvSciBufAttrList reconciledAttrlist = NULL;
        NvSciBufAttrList conflictlist = NULL;
        err = NvSciBufAttrListReconcile(unreconciledAttrlist,
                                        index,
                                        &reconciledAttrlist,
                                        &conflictlist);
        ...
        for (uint32_t i = 0; i < index;  i++) {
            if (unreconciledAttrlist[i] != NULL) {
                NvSciBufAttrListFree(unreconciledAttrlist[i]);
            }
        }
                                                   ...
                                                   bool bFileWrite)
    {
        ...
        NvSciBufAttrKeyValuePair attrKvp = { NvSciBufGeneralAttrKey_RequiredPerm,
                                             &accessPerm,
                                             sizeof(accessPerm) };
        err = NvSciBufAttrListSetAttrs(unreconciledAttrlist[index], &attrKvp, 1);
        CHK_NVSCISTATUS_AND_RETURN(err, "NvSciBufAttrListSetAttrs");

        if (m_isParkingStream) {
            auto status = IMX728IMX623SurfTypeOverride(pip, outType, unreconciledAttrlist[index]);
            if (status != NVSIPL_STATUS_OK) {
            ...
            if (outType == INvSIPLClient::ConsumerDesc::OutputType::ISP2) {
                ...
                auto status = OverrideImageAttributes(unreconciledAttrlist[index], FMT_YUV_420SP_UINT8_BL);
                if (status != NVSIPL_STATUS_OK) {
                    return status;
                }
            }
        ...
        err = NvSciBufAttrListSetAttrs(unreconciledAttrlist[index], setAttrs, 2);
        CHK_NVSCISTATUS_AND_RETURN(err, "NvSciBufAttrListSetAttrs");

        auto status = m_pCamera->GetImageAttributes(pip, outType, unreconciledAttrlist[index]);
        if (status != NVSIPL_STATUS_OK) {
            LOG_ERR("GetImageAttributes failed\n");
            return status;
        }
        ...
        PrintISPOutputFormat(pip, outType, reconciledAttrlist);
    ...
    SIPLStatus AllocateBuffers(uint32_t pip,
                               std::string platformConfigName,
                               bool isRawEnabled,
                               bool isp0Enabled,
                               bool isp1Enabled,
                               ...
                               bool bTwoPassIsp)
    {
        bool outputEnabled[MAX_OUTPUTS_PER_SENSOR] = {true, isp0Enabled, isp1Enabled, isp2Enabled};
    ...
    SIPLStatus GetModuleAndDevBlkIndex(const uint32_t uSensor, uint32_t &moduleIndex, uint32_t &devBlkIndex)
    {
        ...
        if (!bFound)
        {
            LOG_ERR("Module and DevBlk Index not found for sensor: %u\n", uSensor);
            return NVSIPL_STATUS_NOT_SUPPORTED;
        }
    ...
    virtual SIPLStatus CheckSensorStatus(const uint32_t uSensor)
    {
        uint32_t moduleIndex = 0U;
        uint32_t devBlkIndex = 0U;

        SIPLStatus status = GetModuleAndDevBlkIndex(uSensor, moduleIndex, devBlkIndex);
        if (status != NVSIPL_STATUS_OK)
        {
            return status;
        }
    ...
    OV2311NonFuSaCustomInterface* GetOV2311NonFuSaCustomInterface(const uint32_t uSensor) {
        return static_cast<OV2311NonFuSaCustomInterface*>(m_customInterface[uSensor]);
    }

    IMX728_CustomInterface* GetIMX728_CustomInterface(const uint32_t uSensor) {
        return static_cast<IMX728_CustomInterface*>(m_customInterface[uSensor]);
    }

    IMX623_CustomInterface* GetIMX623_CustomInterface(const uint32_t uSensor) {
        return static_cast<IMX623_CustomInterface*>(m_customInterface[uSensor]);
    }

    AR0820NonFuSaCustomInterface* GetAR0820NonFuSaCustomInterface(const uint32_t uSensor) {
        return static_cast<AR0820NonFuSaCustomInterface*>(m_customInterface[uSensor]);
    }

    MAX96712FusaNvCustomInterface* GetMAX96712FusaNVCustomInterface(const uint32_t devblkIdx) {
        IInterfaceProvider* deserializerInterfaceProvider = nullptr;
        MAX96712FusaNvCustomInterface* max96712FusaNvCustomInterface = nullptr;

        // Get the interface provider
        SIPLStatus status = m_pCamera->GetDeserializerInterfaceProvider(devblkIdx,
                                                                        deserializerInterfaceProvider);
        if (status != NVSIPL_STATUS_OK) {
            LOG_ERR("Error %d while getting the deserializer interface provider for Devblk ID 3\n",
                status);
        } else if (deserializerInterfaceProvider != nullptr) {
            // Get the custom interface and cast
            max96712FusaNvCustomInterface = static_cast<MAX96712FusaNvCustomInterface*>
                (deserializerInterfaceProvider->GetInterface(MAX96712_FUSA_NV_CUSTOM_INTERFACE_ID));
            if (max96712FusaNvCustomInterface != nullptr) {
                // Verify that the ID matches expected - we have the correct custom interface
                if (max96712FusaNvCustomInterface->getInstanceInterfaceID() ==
                    MAX96712_FUSA_NV_CUSTOM_INTERFACE_ID) {
                    LOG_DBG("MAX96712 Fusa nv custom interface found\n");
                } else {
                    LOG_ERR("Incorrect interface obtained from module\n");
                    // Set the return pointer to NULL because the obtained pointer
                    // does not point to correct interface.
                    max96712FusaNvCustomInterface = nullptr;
                }
            }
        }

        return max96712FusaNvCustomInterface;
    }
 ...
 private:
    ...
    std::unique_ptr<NvSciSyncObj, CloseNvSciSyncObj> m_sciSyncObjs[MAX_SENSORS][MAX_OUTPUTS_PER_SENSOR];
