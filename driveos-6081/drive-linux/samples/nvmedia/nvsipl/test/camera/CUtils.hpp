...
static const uint32_t MAX_SENSORS = 16U;
static const uint32_t MAX_OUTPUTS_PER_SENSOR = 4U;
...
public:
    ...
    virtual void GetBuffers(std::vector<NvSciBufObj>& vBuffers) = 0;
    virtual SIPLStatus WfdClearDisplay() =0;
    virtual SIPLStatus WfdFlip(NvSciBufObj& bufObj) = 0;
...
{
public:
    static SIPLStatus CreateScratchBuffer(NvSciBufModule &bufModule,
                                          NvSciBufAttrList &bufAttrList,
                                          uint32_t width,
                                          uint32_t height,
                                          NvSciBufObj *pBufObj,
                                          bool isRGBIR);
    static SIPLStatus ConvertRaw(NvSciBufObj srcBufObj,
                                 uint8_t *pSrcBuf,
                                 NvSciBufObj dstBufObj,
                                 uint8_t *pDstBuf);
    static SIPLStatus IsRawBuffer(NvSciBufObj bufObj, bool &bIsRaw);
    static SIPLStatus IsRGBIRBuffer(NvSciBufObj bufObj, bool &bIsRGBIR);
