                            ...
                            bool soloInput)
{
    ...
    uint32_t g = 0U;
    for (uint32_t i = 0U; i < uNumDisplays; i++) {
        status = m_helpers[i]->Init(pRect, m_sciBufModule, m_cpuWaitContext, soloInput);
        if (status != NVSIPL_STATUS_OK) {
...
SIPLStatus CComposite::RegisterNvSciBufObj(uint32_t id, NvSciBufObj &bufObj)
{
    ...
    if (isRawBuffer) {
        if (pInput->m_scratchRegisteredWith2d) {
        ...
        status = pInput->GetScratchBuffer(bufObjToRegister);
        if (status != NVSIPL_STATUS_OK) {
            LOG_ERR("Compositor: GetScratchBuffer failed for source:%u\n", id);
            return status;
        }
    }
    ...
    for (uint32_t i = 0U; i < MAX_SUPPORTED_DISPLAYS; i++) {
        if (m_helpers[i]  != nullptr) {
            ...
            if (isRawBuffer) {
                pInput->m_scratchRegisteredWith2d = true;
            }
        }
    }
...
SIPLStatus CComposite::UnregisterNvSciBufObj(uint32_t id, NvSciBufObj &bufObj)
{
    ...
    if (isRawBuffer) {
        ...
        SIPLStatus status = pInput->GetScratchBuffer(bufObjToUnregister);
        if (status != NVSIPL_STATUS_OK) {
            LOG_ERR("Compositor: GetScratchBuffer failed for source:%u\n", id);
            return status;
        }
    ...
    for (uint32_t i = 0U; i < MAX_SUPPORTED_DISPLAYS; i++) {
        if (m_helpers[i]  != nullptr) {
            ...
            if (isRawBuffer) {
                pInput->m_scratchRegisteredWith2d = false;
            }
        }
    }
