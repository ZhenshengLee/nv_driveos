...
SIPLStatus CCompositeNvSci::SetupStreaming(void)
{
    ...
    if (status != NVSIPL_STATUS_OK) {
        LOG_ERR("Consumer: Compositor: FillNvSciBufAttrList failed\n");
        return status;
    }
    ...
    // send to producer.
    for(std::unique_ptr<ConsumerStream> &pStream : m_usedStreams) {
       NvSciSyncAttrList unreconciledList[3];
       ...
       unreconciledList[0] = signalerAttrList;
        ...
        sciErr = NvSciSyncAttrListCreate(m_sciSyncModule, &unreconciledList[2]);
        CHK_NVSCISTATUS_AND_RETURN(sciErr, "Producer CPU NvSciSyncAttrListCreate");
        sciErr = NvSciSyncAttrListSetAttrs(unreconciledList[2], keyVals, 2);
        CHK_NVSCISTATUS_AND_RETURN(sciErr, "CPU waiter NvSciSyncAttrListSetAttrs");

       sciErr = NvSciSyncAttrListReconcile(unreconciledList, 3, &reconciledList, &newConflictList);
       ...
       NvSciSyncAttrListFree(unreconciledList[2]);
