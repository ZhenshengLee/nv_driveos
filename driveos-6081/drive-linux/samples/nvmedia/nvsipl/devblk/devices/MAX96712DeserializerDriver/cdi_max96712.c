    ...
    const bool embDataType)
{
    ...
    for (i = 0u; i < MAX96712_MAX_NUM_PHY; i++) {
        if (dataType == CDI_MAX96712_DATA_TYPE_RAW12) {
            ...
            if (embDataType) {
                ADD_ONE_REG_FIELD_RET_ERR(REG_FIELD_MODE_DT_PHY0 + i, 1u);
            }
    ...
    for (i = 0u; i < MAX96712_MAX_NUM_PHY; i++) {
        if (dataType == CDI_MAX96712_DATA_TYPE_RAW12) {
            if (drvHandle->ctx.cfgPipeCopy > CDI_MAX96712_CFG_PIPE_COPY_NONE) {
                ADD_ONE_REG_FIELD_RET_ERR(REG_FIELD_MODE_DT_PIPE4 + i, 1u);
            }
        }
    }
    ACCESS_REG_FIELD_RET_ERR(REG_READ_MOD_WRITE_MODE);
    ...
    for (i = 0U; i < MAX96712_NUM_VIDEO_PIPELINES; i++) {
        if (!(drvHandle->ctx.pipelineEnabled & (0x1 << i))) {
            if ((drvHandle->ctx.mipiOutMode == CDI_MAX96712_MIPI_OUT_4x2) ||
               (drvHandle->ctx.mipiOutMode == CDI_MAX96712_MIPI_OUT_4a_2x2)) {
    ...
    DevBlkCDIDevice *handle)
{
    ...
    DevBlkCDII2CReg replicationRegs[] = {
        {0x08A9, 0},  /* For the replication from Tegra A to Tegra B */
        {0x08AA, 0},  /* For the replication from Tegra A to Tegra C */
    };
    DevBlkCDII2CRegList replication = {
        .regs = replicationRegs,
        .numRegs = I2C_ARRAY_SIZE(replicationRegs),
    };
    drvHandle->ctx.tpgEnabled = true;
    ...
    /* enable the replicaiton mode */
    if (drvHandle->ctx.cfgPipeCopy > CDI_MAX96712_CFG_PIPE_COPY_NONE) {
        if (mipiOutMode == CDI_MAX96712_MIPI_OUT_4x2) {
            replicationRegs[0].data = MAX96712_REPLICATION(CDI_MAX96712_TXPORT_PHY_C, CDI_MAX96712_TXPORT_PHY_E);
        } else if ((mipiOutMode == CDI_MAX96712_MIPI_OUT_2x4) || (mipiOutMode == CDI_MAX96712_MIPI_OUT_4a_2x2)) {
            replicationRegs[0].data = MAX96712_REPLICATION(CDI_MAX96712_TXPORT_PHY_D, CDI_MAX96712_TXPORT_PHY_E);
        }
        replicationRegs[0].data |= (1U << 7U);

        if (drvHandle->ctx.cfgPipeCopy == CDI_MAX96712_CFG_PIPE_COPY_MODE_2) {
            if (mipiOutMode == CDI_MAX96712_MIPI_OUT_4x2) {
                replicationRegs[1].data = MAX96712_REPLICATION(CDI_MAX96712_TXPORT_PHY_C, CDI_MAX96712_TXPORT_PHY_F);
            } else if ((mipiOutMode == CDI_MAX96712_MIPI_OUT_2x4) || (mipiOutMode == CDI_MAX96712_MIPI_OUT_4a_2x2)) {
                replicationRegs[1].data = MAX96712_REPLICATION(CDI_MAX96712_TXPORT_PHY_D, CDI_MAX96712_TXPORT_PHY_F);
            }
            replicationRegs[1].data |= (1U << 7U);
        }

        status = DevBlkCDII2CPgmrWriteArray(drvHandle->i2cProgrammer, &replication);
        if (status != NVMEDIA_STATUS_OK) {
            return status;
        }
    }
    ...
    PHYModeMAX96712 phyMode)
{
    ...
    if ((drvHandle->ctx.revision != CDI_MAX96712_REV_1) &&
        ...
        ((mipiOutMode == CDI_MAX96712_MIPI_OUT_2x4) || (mipiOutMode == CDI_MAX96712_MIPI_OUT_4a_2x2))) {
    ...
    mipiOutputReg.data = (drvHandle->ctx.cfgPipeCopy > CDI_MAX96712_CFG_PIPE_COPY_MODE_1) ? 0x44 : 0xE4;
    status = DevBlkCDII2CPgmrWriteUint8(drvHandle->i2cProgrammer,
                                        mipiOutputReg.address,
                                        mipiOutputReg.data);
    if (status != NVMEDIA_STATUS_OK) {
