...
SIPLStatus CNvMMax96712::DoSetConfig(const DeserInfo *deserInfo, DeserializerParams *params)
{
    ...
    m_sDeserializerName = deserInfo->name;
...
SIPLStatus CNvMMax96712::SetMAX96712Ctx()
{
    ...
    ctx->cfgPipeCopy = CDI_MAX96712_CFG_PIPE_COPY_NONE;
#if !NV_IS_SAFETY || SAFETY_DBG_OV
    if (m_sDeserializerName == "MAX96712_camRecCfg_V1" ||
        m_sDeserializerName == "MAX96722_camRecCfg_V1") {
        ctx->cfgPipeCopy = CDI_MAX96712_CFG_PIPE_COPY_MODE_1;
    } else if (m_sDeserializerName == "MAX96712_camRecCfg_V2" ||
               m_sDeserializerName == "MAX96722_camRecCfg_V2") {
    ...
    switch (m_eInterface) {
        ...
        case NVSIPL_CAP_CSI_INTERFACE_TYPE_CSI_GH:
            ctx->txPort = (ctx->i2cPort == CDI_MAX96712_I2CPORT_0) ? CDI_MAX96712_TXPORT_PHY_D :
                                                                     CDI_MAX96712_TXPORT_PHY_E;
            ctx->mipiOutMode =
                (ctx->cfgPipeCopy == CDI_MAX96712_CFG_PIPE_COPY_MODE_2) ? CDI_MAX96712_MIPI_OUT_4a_2x2 :
                                                                          CDI_MAX96712_MIPI_OUT_2x4;
            for (uint8_t i = 0U; i < 2U; i++) {
                ctx->lanes[i] = 4U;
            }
            for (uint8_t i = 2U; i < MAX96712_MAX_NUM_PHY; i++) {
                ctx->lanes[i] = (ctx->cfgPipeCopy == CDI_MAX96712_CFG_PIPE_COPY_MODE_2) ? 2U : 4U;
            }
            break;
#if !NV_IS_SAFETY
...
const char** CNvMDeserializer_GetName() {
    static const char* names[] = {
        "MAX96712",
        "MAX96722",
        "MAX96712_camRecCfg_V1", /* Samtec cable V1 */
        "MAX96712_camRecCfg_V2", /* Samtec cable V2 */
        "MAX96722_camRecCfg_V1", /* Samtec cable V1 */
        "MAX96722_camRecCfg_V2", /* Samtec cable V2 */
        NULL
    };
