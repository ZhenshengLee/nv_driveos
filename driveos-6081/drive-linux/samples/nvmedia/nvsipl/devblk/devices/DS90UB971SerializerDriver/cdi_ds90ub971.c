    ...
    DevBlkCDIDevice *handle)
{
    ...
    DevBlkCDII2CReg ds90ub971_default_config_regs[] = {
        {0x02, 0x73}, // Set CSI 4 lane continuous clock
        {0x4B, 0x02},
        {0x49, 0x06},
        {0x0A, 0x12},
    };
    DevBlkCDII2CRegList ds90ub971_default_config = {
        .regs = ds90ub971_default_config_regs,
        .numRegs = (uint32_t)I2C_ARRAY_SIZE(ds90ub971_default_config_regs),
    };

    status = DevBlkCDII2CPgmrWriteArray(drvHandle->i2cProgrammer,
                            &ds90ub971_default_config);
...
static NvMediaStatus
SensorReset(DevBlkCDIDevice const *handle,
    GPIODS90UB971 gpio,
    bool level)
{
    _DriverHandle *drvHandle = (_DriverHandle *)handle->deviceDriverHandle;
    ...
    DevBlkCDII2CReg ds90ub971_sensor_reset_regs[] = {
        {0x0D, 0x00, 10000},
        {0x0E, 0x0F, 10000},
        {0x0D, 0x00, 10000},
        {0x0D, 0x00, 10000},
    ...
    DevBlkCDII2CRegList ds90ub971_sensor_reset = {
        .regs = ds90ub971_sensor_reset_regs,
        .numRegs = (uint32_t)I2C_ARRAY_SIZE(ds90ub971_sensor_reset_regs),
    };
    ...
    if (level) {
        ds90ub971_sensor_reset_regs[0].data |= (1 << gpio);
        ds90ub971_sensor_reset_regs[1].data |= ((1 << gpio) << 4);
        ds90ub971_sensor_reset_regs[1].data &= ~(1 << gpio);
        ds90ub971_sensor_reset_regs[2].data &= ~(1 << gpio);
        ds90ub971_sensor_reset_regs[3].data |= (1 << gpio);
    } else {
        ds90ub971_sensor_reset_regs[0].data &= ~(1 << gpio);
        ds90ub971_sensor_reset_regs[1].data |= ((1 << gpio) << 4);
        ds90ub971_sensor_reset_regs[1].data &= ~(1 << gpio);
    ...
    void *parameter)
{
    ...
    switch (parameterType) {
        ...
        case CDI_WRITE_PARAM_CMD_DS90UB971_SET_FSYNC_GPIO:
            ...
            break;
        case CDI_WRITE_PARAM_CMD_DS90UB971_SENSOR_RESET:
            if(parameterSize == sizeof(param->localGpio)) {
                isValidSize = true;
                status = SensorReset(handle,
