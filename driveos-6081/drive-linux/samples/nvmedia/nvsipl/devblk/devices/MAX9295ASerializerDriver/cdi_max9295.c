    ...
    bool embDataType)
{
    ...
    DevBlkCDII2CReg doubleBpp12Bpp8Regs[] = {
                        {0x0313, 0x40}, /* Double 12-bit data in pipe Z */
                        {0x0337, 0x20}, /* Triple 8-bit data in pipe Z */
                        {0x031E, 0x38}, /* BPP = 24 in pipe Z */
                        {0x0112, 0x0E}, /* Limit heartbeat in pipe Z */
    };
    ...
    /* Update settings for RAW 10 */
    if (dataType == CDI_MAX9295_DATA_TYPE_RAW10) {
        mappingPixelRegs[0].data = 0x6B;   /* Route 10bit RAW to VIDEO_Z (MSB enable) */
        doubleBpp12BppRegs[0].data = 0x04; /* Double 10-bit data on pipe Z */
        doubleBpp12BppRegs[1].data = 0x34; /* Min BPP = 20 in pipe Z */
        doubleBpp12BppRegs[2].data = 0xA;  /* Use default heartbeat on pipe Z */
    }
...
static NvMediaStatus
IncreaseCMURegulatorVoltage(
    DevBlkCDIDevice *handle)
{
    NvMediaStatus status = NVMEDIA_STATUS_OK;
    _DriverHandle *drvHandle = NULL;

    DevBlkCDII2CReg CMURegVolRegs[] = {{0x0302, 0x10}};
    DevBlkCDII2CRegList CMURegVol = {
        .regs = CMURegVolRegs,
        .numRegs = I2C_ARRAY_SIZE(CMURegVolRegs),
    };
    ...
    status = DevBlkCDII2CPgmrWriteArray(drvHandle->i2cProgrammer, &CMURegVol);
    if (NVMEDIA_STATUS_OK != status) {
        SIPL_LOG_ERR_STR_INT("MAX9295: Failed to increase CMU regulator voltage", (int32_t)status);
    }
    ...
    DevBlkCDIDevice *handle)
{
    ...
    if (drvHandle->rev == CDI_MAX9295_REV_8) {
        status = IncreaseCMURegulatorVoltage(handle);
    }
