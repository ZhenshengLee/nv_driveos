...
/**
 *  @brief left shift 1U by number of bits
 *
 *  A utility function to produce single-bit values.
 *
 *  @param[in] b uint32_t number of bits to be shifted
 *
 *  @retval uint32_t value after shifting, 0U if \p bits is 32U or greater
 *
 */
static inline uint32_t bit32(uint32_t const b)
{
    uint32_t result = 0U;
 ...
 *
 *  Adding two uint16_t values together and wrap in case of overflow
 *
 *  @param[in] a  uint16_t value to be added
 *  @param[in] b  uint16_t value to be added
 *
 *  @retval the uint16_t wrapped addition
 */
static inline uint16_t wrapAddUint16(uint16_t const a, uint16_t const b)
{
    uint16_t ret = 0U;
    if ((a > 0U) && ((uint16_t)(a - 1U) > ((uint16_t)(UINT16_MAX) - b))) {
        ret = (uint16_t)((a - 1U) - ((uint16_t)(UINT16_MAX) - b));
    }
    else {
        ret = a + b;
    }
 ...
 *  @brief addition of two unsigned integers without MISRA C++ violation
 *
 *  Adding two uint32_t values together and wrap in case of overflow
 *
 *  @param[in] a  uint32_t value to be added
 *  @param[in] b  uint32_t value to be added
 *
 *  @retval the uint32_t wrapped addition
 */
static inline uint32_t wrapAddUint32(uint32_t const a, uint32_t const b)
{
    uint32_t ret = 0U;
    if ((a > 0U) && ((uint32_t)(a - 1U) > ((uint32_t)(UINT32_MAX) - b))) {
        ret = (a - 1U) - ((uint32_t)(UINT32_MAX) - b);
    }
    else {
        ret = a + b;
    }
...
/**
 *  @brief Incrementing unsigned integer
 *
 *  Increment uint32_t value and clamp to UINT32_MAX
 *
 *  @param[in] a uint32_t value to be incremented
 *
 *  @retval the uint32_t saturated incrementing clamped to UINT32_MAX
 */
static inline uint32_t saturatingInc(uint32_t a)
{
    uint32_t ret;
    if (a >= UINT32_MAX) {
        ret = UINT32_MAX;
 ...
 *
 * @param[in] x uint8_t value to be converted to uint32_t type
 *
 * @retval input parameter x cast to uint32_t
 */
static inline uint32_t toUint32FromUint8(uint8_t const x) {
    uint32_t const ret = (uint32_t)(x);
    return ret;
}

/**
 * @brief Convert uint16_t to uint32_t
 *
 * @param[in] x uint16_t value to be converted to uint32_t type
 *
 * @retval input parameter x cast to uint32_t
 */
static inline uint32_t toUint32FromUint16(uint16_t const x) {
    uint32_t const ret = (uint32_t)(x);
    return ret;
}
...
/**
 * Left shifts the number of specfied bits for uint8_t input val. Ensures no
 * undefined behaviour occurs.
 * dest are non-NULL.
 *
 * @param[in]  val         Value to be shifted.
 * @param[in]  shift_count Number of bits to be shifted.
 * @retval ret input parameter val left shifted by bits <= UINT8_MAX.
 *
 */

static inline uint8_t lshift8(uint8_t const val, uint32_t const shift_count)
{
    uint8_t ret = 0U;
    if (shift_count < 8U) {
        uint8_t mask = 0xFFU;
        ret = (val << shift_count) & mask;
    }
    return ret;
}

/**
 * Left shifts by the number of specfied bits for uint16_t input val. Ensures
 * no undefined behaviour occurs.
 * dest are non-NULL.
 *
 * @param[in]  val         Value to be shifted.
 * @param[in]  shift_count Number of bits to be shifted.
 * @retval ret input parameter val left shifted by bits <= UINT16_MAX.
 *
 */

static inline uint16_t lshift16(uint16_t const val, uint32_t const shift_count)
{
    uint16_t ret = 0U;
    if (shift_count < 16U) {
...
/**
 * Left shifts the number of specfied bits for uint32_t input val. Ensures no
 * undefined behaviour occurs.
 * dest are non-NULL.
 *
 * @param[in]  val         Value to be shifted.
 * @param[in]  shift_count Number of bits to be shifted.
 * @retval ret input parameter val left shifted by bits <= UINT32_MAX.
 *
 */

static inline uint32_t lshift32(uint32_t const val, uint32_t const shift_count)
{
    uint32_t ret = 0U;
    if (shift_count < 32U) {
...
/**
 * Left shifts the number of specfied bits for uint64_t input val. Ensures no
 * undefined behaviour occurs.
 * dest are non-NULL.
 *
 * @param[in]  val         Value to be shifted.
 * @param[in]  shift_count Number of bits to be shifted.
 * @retval ret input parameter val left shifted by bits <= UINT64_MAX.
 */
static inline uint64_t lshift64(uint64_t const val, uint32_t const shift_count)
{
    uint64_t ret = 0U;
    if (shift_count < 64U) {
...
/**
 * Right shifts the number of specfied bits for uint8_t input val. Ensures no
 * undefined behaviour occurs.
 * dest are non-NULL.
 *
 * @param[in]  val         Value to be shifted.
 * @param[in]  shift_count Number of bits to be shifted.
 * @retval ret input parameter val right shifted by bits <= UINT8_MAX.
 *
 */

static inline uint8_t rshift8(uint8_t const val, uint32_t const shift_count)
{
    uint8_t ret = 0U;
    if (shift_count < 8U) {
        uint8_t mask = 0xFFU;
...
/**
 * Right shifts by the number of specfied bits for uint16_t input val. Ensures
 * no undefined behaviour occurs.
 * dest are non-NULL.
 *
 * @param[in]  val         Value to be shifted.
 * @param[in]  shift_count Number of bits to be shifted.
 * @retval ret input parameter val right shifted by bits <= UINT16_MAX.
 *
 */

static inline uint16_t rshift16(uint16_t const val, uint16_t const shift_count)
{
    uint16_t ret = 0U;
    if (shift_count < 16U) {
        uint16_t mask = 0xFFFFU;
...
/**
 * Right shifts the number of specfied bits for uint32_t input val. Ensures no
 * undefined behaviour occurs.
 * dest are non-NULL.
 *
 * @param[in]  val         Value to be shifted.
 * @param[in]  shift_count Number of bits to be shifted.
 * @retval ret input parameter val right shifted by bits <= UINT32_MAX,
 *
 */

static inline uint32_t rshift32(uint32_t const val, uint32_t const shift_count)
{
    uint32_t ret = 0U;
    if (shift_count < 32U) {
        uint32_t mask = 0xFFFFFFFFU;
...
/**
 * Left shifts 1U by the number of specfied bits. Ensures no undefined
 * behaviour occurs.
 * dest are non-NULL.
 *
 * @param[in]  shift_count Number of bits to be shifted.
 * @retval ret returns lshift8 output of 1 left shifted by shift_count <= UINT8_MAX.
 *
 */

static inline uint8_t bit8(uint32_t const shift_count)
{
    return lshift8(1U, shift_count);
}
 ...
 *
 * @param[in]  shift_count Number of bits to be shifted.
 * @retval     ret returns lshift16 output of 1 left shifted by shift_count <= UINT16_MAX.
 *
 */

static inline uint16_t bit16(uint32_t const shift_count)
{
    return lshift16(1U, shift_count);
}
 ...
 *
 * @param[in]  shift_count Number of bits to be shifted.
 * @retval     ret returns lshift64 output of 1 left shifted by shift_count <= UINT64_MAX.
 */
static inline uint64_t bit64(uint32_t const shift_count)
{
    return lshift64(1U, shift_count);
}

/**
 * \brief For converting csi Type to corresponding csi Port.
 *
 * \param[in]   csiType      The CSI port Type defined in \ref NvSiplCapInterfaceType.
 *
 ...
 *          - \ref NVSIPL_CAP_CSI_INTERFACE_TYPE_CSI_A1
 *          - \ref NVSIPL_CAP_CSI_INTERFACE_TYPE_CSI_B1
 *          - \ref NVSIPL_CAP_CSI_INTERFACE_TYPE_CSI_C1
 *          - \ref NVSIPL_CAP_CSI_INTERFACE_TYPE_CSI_D1
 *          - \ref NVSIPL_CAP_CSI_INTERFACE_TYPE_CSI_E1
 *          - \ref NVSIPL_CAP_CSI_INTERFACE_TYPE_CSI_F1
 *          - \ref NVSIPL_CAP_CSI_INTERFACE_TYPE_CSI_G1
 *          - \ref NVSIPL_CAP_CSI_INTERFACE_TYPE_CSI_H1
 */
static inline int32_t CsiPortForType(NvSiplCapInterfaceType const csiType)
{
    int32_t csiPort = -1;
    switch(csiType) {
