...
CNvMSensor::CNvMSensor(): CNvMDevice(), ISensorControl(), m_pipelineIndex {0U},
    ...
    m_bIsAuthEnabled {false}, m_imageAuthThreadID {0U}
{
...
SIPLStatus CNvMSensor::SetConfig(SensorInfo const* const sensorInformation, DeviceParams const* const params)
{
    ...
    if ((!exitFlag) && (m_eState != DeviceState::CREATED)) {
...
void CNvMSensor::SetImgAuthThreadID(uint8_t const threadID) noexcept
{
    m_imageAuthThreadID = threadID;
}

uint8_t CNvMSensor::GetImgAuthThreadID() const noexcept
{
    return m_imageAuthThreadID;
}
