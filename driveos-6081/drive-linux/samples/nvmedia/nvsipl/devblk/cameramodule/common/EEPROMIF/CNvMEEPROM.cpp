...
SIPLStatus CNvMEEPROM::SetConfig(EEPROMInfo const *const info, DeviceParams const *const params)
{
...
SIPLStatus CNvMEEPROM::ReadData(std::uint16_t const address,
                                ...
                                std::uint8_t * const buffer)
{
    ...
    } else if (m_eState == DeviceState::STARTED) {
...
/* coverity[misra_cpp_2008_rule_16_2_1_violation] : intentional TID-1391 */
#if !NV_IS_SAFETY
SIPLStatus CNvMEEPROM::WriteData(std::uint16_t const address,
                                 std::uint32_t const length,
                                 std::uint8_t * const buffer)
{
    SIPLStatus status {NVSIPL_STATUS_OK};
