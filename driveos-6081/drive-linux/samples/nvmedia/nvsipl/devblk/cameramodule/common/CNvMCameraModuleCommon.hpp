...
enum FPDLinkMode
{
    LINKMODE_FPDLINK4_SYNC = 0U,
    LINKMODE_FPDLINK3_SYNC,
};
#endif
    ...
    typedef struct {
        typedef struct {
...
#if !NV_IS_SAFETY
                uint8_t M;              /*!< M Value for M/N divider for CLKOUT (fpdlink only) */
                uint8_t N;              /*!< N Value for M/N divider for CLKOUT (fpdlink only) */
#endif
            } RefClock;
            ...
            typedef struct {
                ...
                bool isLanePolarityConfigureNeeded {false};
...
/* coverity[misra_cpp_2008_rule_16_2_1_violation] : intentional TID-1391 */
            ...
            /**
             * Holds the List of Serializer GPIO pins to run Toggle test.
             */
            std::vector<uint8_t> vSerGPIOsToggleTest;

            /**
             * Holds the List of Serializer GPIO pins to run Open Detection
             * Test prior to sensor init.
             */
            std::vector<uint8_t> vSerGPIOsOpenDetection;

            /**
             * Hold Skip registers list for serializer's register CRC computation
             */
            std::vector<uint16_t> serializerCRCSkipRegisterList;

#if !NV_IS_SAFETY
            /**
             * Holds the FPD-Link mode
             */
            uint8_t FPDLinkMode;
#endif
