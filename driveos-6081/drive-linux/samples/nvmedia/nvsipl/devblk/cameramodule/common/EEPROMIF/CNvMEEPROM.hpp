...
public:
    ...
    */
     ...
     */
    virtual SIPLStatus SetConfig(EEPROMInfo const *const info, DeviceParams const *const params);
    ...
    void SetStreamingState(bool const streaming) {
        if (streaming == true) {
            m_eState = DeviceState::STARTED;
        } else {
            m_eState = DeviceState::STOPPED;
        }
    }
     ...
     */
    virtual SIPLStatus ReadData(std::uint16_t const address,
