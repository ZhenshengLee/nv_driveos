...
SIPLStatus CNvMMAX96712_9295_OV2311::CheckModuleStatus()
{
    SIPLStatus status = NVSIPL_STATUS_OK;
    NvMediaStatus mediaStatus = NVMEDIA_STATUS_OK;
    CNvMDeserializer::LinkAction linkAction;
    std::vector<CNvMDeserializer::LinkAction> allLinkActions;

    status = DoSetPower(true);
    if (status != NVSIPL_STATUS_OK) {
        SIPL_LOG_ERR_STR_INT("SetPower failed with SIPL error %d", static_cast<int32_t>(status));
        goto done;
    }
    ...
    linkAction.linkIdx = m_linkIndex;
    linkAction.eAction = CNvMDeserializer::LinkAction::Action::LINK_ENABLE;
    allLinkActions.push_back(linkAction);

    status = m_pDeserializer->ControlLinks(allLinkActions);
    if (status != NVSIPL_STATUS_OK) {
        SIPL_LOG_ERR_STR_INT("ControlLinks in CNvMDeviceBlock::EnableLink failed with SIPL error",
    ...
    mediaStatus = MAX96712CheckLink(m_pDeserializer->GetCDIDeviceHandle(),
                                    ...
                                    false);
    if (mediaStatus != NVMEDIA_STATUS_OK) {
        SIPL_LOG_ERR_STR("Failed to lock the link\n");
        status = ConvertNvMediaStatus(mediaStatus);

        /* Turn the camera module power off */
        SIPLStatus statusPwr = DoSetPower(false);
        if (statusPwr != NVSIPL_STATUS_OK) {
            SIPL_LOG_ERR_STR_INT("SetPower failed with SIPL error %d", static_cast<int32_t>(statusPwr));
            status = statusPwr;
        }
    }
