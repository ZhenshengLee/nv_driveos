...
SIPLStatus CNvMTransportLink_Max96712_9295::SetupAddressTranslations(DevBlkCDIDevice* brdcstSerCDI)
{
    ...
    // Set serializer default
    LOG_INFO("Set the default setting in the serializer\n");
    nvmStatus = MAX9295SetDefaults(brdcstSerCDI);
    if (nvmStatus != NVMEDIA_STATUS_OK) {
        SIPL_LOG_ERR_STR_INT("MAX9295: MAX9295SetDefaults(brdcstSerCDI) failed with NvMedia error", (int32_t)nvmStatus);
        return ConvertNvMediaStatus(nvmStatus);
    }
