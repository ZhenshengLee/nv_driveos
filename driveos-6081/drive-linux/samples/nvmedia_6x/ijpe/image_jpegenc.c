...
)
{
    ...
    err = NvSciSyncAttrListCreate(syncModule, &cpu_signalerAttrList);
    if(err != NvSciError_Success) {
        LOG_ERR("%s: Create cpu signaler attr list failed. Error: %d \n", __func__, err);
        goto fail;
    }
    ...
    err = NvSciSyncAttrListCreate(syncModule, &nvm_waiterAttrList);
    if(err != NvSciError_Success) {
        LOG_ERR("%s: Create nvm waiter attr list failed. Error: %d \n", __func__, err);
        status = NVMEDIA_STATUS_ERROR;
        goto fail;
    }

    /* Fill NvSciSyncAttrList for nvmedia waiter */
    nvmstatus = NvMediaIJPEFillNvSciSyncAttrList(ijpeCtx, nvm_waiterAttrList, NVMEDIA_WAITER);
    if(status != NVMEDIA_STATUS_OK) {
       LOG_ERR("main: Failed to fill signaler attr list.\n");
       goto fail;
    }
    ...
    err = NvSciSyncAttrListReconcile(unreconciledList, 2, &reconciledList,
            &newConflictList);
    if(err != NvSciError_Success) {
        LOG_ERR("NvSciSyncAttrListReconcile failed\n");
        status = NVMEDIA_STATUS_ERROR;
        goto fail;
    }

    /* Create NvSciSync object and get the syncObj */
    err = NvSciSyncObjAlloc(reconciledList, preSyncObj);
    if(err != NvSciError_Success) {
        LOG_ERR("NvSciSyncObjAlloc failed\n");
        status = NVMEDIA_STATUS_ERROR;
        goto fail;
    }
fail:
    ...
    if (NULL != cpu_signalerAttrList) {
        NvSciSyncAttrListFree(cpu_signalerAttrList);
    }
    if (NULL != nvm_waiterAttrList) {
        NvSciSyncAttrListFree(nvm_waiterAttrList);
    }
...
int main(int argc, char *argv[])
{
    ...
    NvSciSyncObj eofSyncObj = {0}, preSyncObj = {0};
    NvSciSyncModule syncModule = NULL;
    NvSciSyncCpuWaitContext cpuWaitContext = NULL;
    ChromaFormat chromaFormat = YUV420SP_8bit;
    ...
    if (args.pixelFormat) {
        chromaFormat = YUV420P_8bit;
        planeCount = 3;
    }
    else {
        // default NV12
        chromaFormat = YUV420SP_8bit;
        planeCount = 2;
    }
    ...
    if (args.alternateCreateAPI) {
        ...
        LOG_DBG("Using NvMediaIJPECreate API to create JPEG ENCODER instance \n");
    }
    else {
        ijpeCtx = NvMediaIJPECreateEx(args.maxOutputBuffering,        // maxOutputBuffering
                                      MAX_BITSTREAM_SIZE,             // maxBitstreamBytes
                                      args.instanceId);               // HW instance id
        LOG_DBG("Using NvMediaIJPECreateEx API to create JPEG ENCODER instance \n");
    }
    if(!ijpeCtx) {
    ...
    if (args.outputImageSize != 0) {
        attr.targetImageSize = args.outputImageSize;
        status = NvMediaIJPESetAttributes(ijpeCtx,
    ...
    status = sAllocPreNvSciSyncObj(ijpeCtx, syncModule, &preSyncObj);
    if (status != NVMEDIA_STATUS_OK) {
        LOG_ERR("main: Pre NvSciSyncObj creation failed\n");
        goto fail;
    }
    LOG_DBG("main: Created Pre NvSciSyncObj\n");
    ...
    status = NvMediaIJPERegisterNvSciSyncObj(ijpeCtx, NVMEDIA_PRESYNCOBJ, preSyncObj);
    if(status != NVMEDIA_STATUS_OK) {
        LOG_ERR("main: Failed to register PRE NvSciSyncObj\n");
        goto fail;
    }
    LOG_DBG("main: Registered NvSciSyncObjs\n");
    ...
    status = NvMediaIJPERegisterNvSciBufObj(ijpeCtx, appBuffer->bufObj);
    if(status != NVMEDIA_STATUS_OK) {
        LOG_ERR("main: Failed to register NvSciBufObj\n");
        goto fail;
    }
    LOG_DBG("main: Registered NvSciBufObj\n");
    ...
    while(nextFrameFlag && (encodeStop == 0u)) {
        ...
        if (err != NvSciError_Success)
        {
            LOG_ERR("main: NvSciSyncObjGenerateFence function failed \n");
            goto fail;
        }
        // Read Frame
        ...
        if (status != NVMEDIA_STATUS_OK)
        {
            LOG_ERR("main: NvMediaIJPDInsertPreNvSciSyncFence function failed \n");
            goto fail;
        }
        ...
        if ((args.outputImageSize) && (args.quantTable)) {
            status = NvMediaIJPEFeedFrameRateControl(ijpeCtx,
                                               appBuffer->bufObj,
                                               attr.lumaQuant,
                                               attr.chromaQuant,
                                               args.outputImageSize,
                                               args.instanceId);
        }
        else if (args.quantTable) {
...
fail:
    ...
    if (preSyncObj) {
        /* Unregister NvSciSyncObj */
        status = NvMediaIJPEUnregisterNvSciSyncObj(ijpeCtx, preSyncObj);
        if (status != NVMEDIA_STATUS_OK) {
            LOG_ERR("%s: Failed to unregister NvSciSyncObj\n", __func__);
        }
    ...
    if(appBuffer->bufObj) {
        ...
        status = NvMediaIJPEUnregisterNvSciBufObj(ijpeCtx, appBuffer->bufObj);
        if (status != NVMEDIA_STATUS_OK) {
            LOG_ERR("%s: Failed to unregister NvSciBufObj\n", __func__);
        }
