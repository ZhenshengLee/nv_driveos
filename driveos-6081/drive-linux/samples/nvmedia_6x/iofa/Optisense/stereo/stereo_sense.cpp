...
int main(int argc, char **argv)
{
    ...
    FileWriter *outFile[5] = {}, *costFile = NULL;
    TestArgs st_args = {};
    ...
    if (!st_args.outputFilename.empty())
    {
        ...
        if (st_args.do_RL ==1)
        {
            ...
            string out2 = st_args.outputFilename.substr(0, pos) + "_down.bin";
            ...
            string out3 = st_args.outputFilename.substr(0, pos) + "_downR.bin";
            outFile[3] =  new FileWriter(out3);
            if (!outFile[3]->initialize())
            {
                cerr << "outFile[3]->initialize failed\n";
                goto exit;
            }
        ...
        {
            int pos = st_args.outputFilename.find(".");
            string out4 = st_args.outputFilename.substr(0, pos) + "_LR_check.bin";
            outFile[4] =  new FileWriter(out4);
            if (!outFile[4]->initialize())
            {
                cerr << "outFile[4]->initialize failed\n";
                goto exit;
            }
        }
    }
    ...
    while(frames < st_args.nframes)
    {
	    ...
	    else
	    {
            ...
            if (outFile[i+2] != NULL)
            {
                if (!outFile[i+2]->writeSurface(outSurface[i]))
                {
        ...
        if (LRcheck != NULL)
        {
            ...
            if (outFile[4] != NULL)
            {
                if (!outFile[4]->writeSurface(LRSurface))
                {
                    cerr << "writeSurface failed for output surface\n";
                    goto exit;
                }
...
exit:
    ...
    if (!testPass) 
    {
        cerr << "total failures: 1 \n";
        return -1;
    }
    else 
    {
        cout << "total failures: 0 \n";
        return 0;
    }
