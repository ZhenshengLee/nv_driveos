...
void LRCheckStereo::stereoLRCheck(uint16_t *src_leftDisp, uint16_t *src_rightDisp, uint16_t *dst, uint16_t threshold)
{
    ...
    for (uint32_t j = 0; j < m_inHeight; j++)
    {
        ...
        {
            leftDisp = (*(src_leftDisp + j*m_inWidth + i) + 15) >> 5;
            leftDisp_value = (*(src_leftDisp + j*m_inWidth + i)) ;
            if ((int)(i-leftDisp) < 0)
            {
                *(dst + j*m_inWidth + i) = 0;
                continue;
            }
            rightDisp_value = (*(src_rightDisp + j*m_inWidth + i - leftDisp));
...
void FBCheckFlow::flowFBCheck(int16_t *src_fwdFlow, int16_t *src_bwdFlow, int16_t *dst, uint16_t threshold)
{
    ...
    int16_t fwdFlow_x, fwdFlow_y, bwdFlow_x, bwdFlow_y;
    ...
    for (int16_t j = 0; j < (int16_t)m_inHeight; j++)
    {
	    ...
	    fwdFlow_x = *(src_fwdFlow + j*srcpitch + 2*i);
            fwdFlow_y = *(src_fwdFlow + j*srcpitch + 2*i +1);
	    ...
	    bwdFlow_x = *(src_bwdFlow + dsty*srcpitch + 2*dstx);
            bwdFlow_y = *(src_bwdFlow + dsty*srcpitch + 2*dstx +1);
            ...
            if((abs(fwdFlow_x+bwdFlow_x) > threshold) || (abs(fwdFlow_y+bwdFlow_y) >threshold))
	    {
               *(dst + j*srcpitch + 2*i) = UNKNOWN_FLOW_THRESH;
               *(dst + j*srcpitch + 2*i +1) = UNKNOWN_FLOW_THRESH;
                continue;
            ...
            *(dst + j*srcpitch + 2*i) = fwdFlow_x;
            *(dst + j*srcpitch + 2*i +1) = fwdFlow_y;
        }
    }
}
