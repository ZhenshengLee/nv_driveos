...
void PrintUsage()
{
    ...
    LOG_MSG("-inputBuffering       input buffering valid range 1 to 8, Default 1 \n");
    ...
    LOG_MSG("-p                    num of passes valid range 1 to 3\n");
    LOG_MSG("-profile              enable profiling support\n");
    LOG_MSG("                      0: disable profiling\n");
    LOG_MSG("                      1: Deprecated option. Should not be used \n");
    LOG_MSG("                      2: Deprecated option. Should not be used \n");
    ...
    LOG_MSG("-d                    Enable diagonal path search in SGM: 0/1. Different values for each pyramid level can be entered \n");
    ...
    LOG_MSG("-profileTestTime      Verifies if latencies are within prescribed time \n");
    LOG_MSG(" [InitLatency]        Provide init latency in microsec \n");
    LOG_MSG(" [SubmitLatency]      Provide submit latency in microsec \n");
    LOG_MSG(" [ExecutionLatency]   Provide execution latency in microsec \n");
    LOG_MSG("                      e.g. -profileTestTime 12000 90 10000. Must provide all 3 latencies when this option is enabled.\n");
    LOG_MSG("-flowMode             Pyd/Epi Mode\n");
    LOG_MSG("                      0: Pyramid SGM mode\n");
    LOG_MSG("                      1: Epipolar SGM mode\n");
    LOG_MSG("-e                    Epipolar info data file required for Epiploar SGM mode. only .txt file extension is supported \n");
    LOG_MSG("-multiEpi             To provide multiple Epipolar files in case of multiframe input.\n"); 
    LOG_MSG("                      Epipolar files should be present at same location of test app executable in this format 000000_epi.txt, 000001_epi.txt\n");
    LOG_MSG("                      0: Disable multiEpi (default) \n");
    LOG_MSG("                      1: Enable multiEpi \n");
    LOG_MSG("-ndisp                Number of disparities (used in Epipolar mode only)\n");
    LOG_MSG("                      128 disparities \n");
    LOG_MSG("                      256 disparities \n");
    LOG_MSG("-epiSearchRange       Search range in pixels in epipolar mode \n");
    LOG_MSG("                      128 pixels \n");
    LOG_MSG("                      256 pixels \n");
...
int ParseArgs(int argc, char *argv[], TestArgs *args)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (argv[i][0] == '-')
        {
            ...
            else if (strcmp(&argv[i][1], "profileTestTime") == 0)
            {
                if (bDataAvailable)
                {
                    args->profile = 3;
                    args->profileTestEnable = true;
                    args->initLat = atoi(argv[++i]);
                    args->submitLat = atoi(argv[++i]);
                    args->execLat = atoi(argv[++i]);
                }
                else
                {
                    LOG_ERR("ParseArgs: -profileTestTime must be followed by init, submit, execution latency values in microsec \n");
                    return -1;
                }
            }
            ...
            else if (strcmp(&argv[i][1], "profile") == 0)
            {
                ...
                if ((args->profile == 1) || (args->profile == 2))
                {
                    LOG_ERR("ERR: -profile values 1 and 2 are deprecated");
                    return -1;
                }
            }
            ...
            else if (strcmp(&argv[i][1], "flowMode") == 0)
            {
                if (!bDataAvailable || !sscanf(argv[++i], "%hhd", &args->flowmode))
                {
                    LOG_ERR("ERR: -flowMode should be followed by 0 or 1. \n");
                    return -1;
                }
            }
            else if (strcmp(&argv[i][1], "e") == 0)
            {
                if (bDataAvailable)
                {
                    args->epipolarFilename = argv[++i];
                }
                else
                {
                    LOG_ERR("ERR: -e must be followed by epipolar info file name \n");
                    return -1;
                }
            }
            else if (strcmp(&argv[i][1], "multiEpi") == 0)
            {
                if (!bDataAvailable || !sscanf(argv[++i], "%hhd", &args->multiEpi))
                {
                    LOG_ERR("ERR: -multiEpi should be followed by 0 or 1. \n");
                    return -1;
                }
            }
            ...
            else if (strcmp(&argv[i][1], "epiSearchRange") == 0)
            {
                if (!bDataAvailable || !sscanf(argv[++i], "%hd", &args->epiSearchRange))
                {
                    LOG_ERR("ERR: -epiSearchRange must be followed by 128 pixels or 256 pixels \n");
                    return -1;
                }
            }
            else if (strcmp(&argv[i][1], "enableVMSuspend") == 0)
            {
#if __QNXNTO__
