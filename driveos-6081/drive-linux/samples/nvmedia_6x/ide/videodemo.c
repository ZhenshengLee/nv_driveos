/*
 * Copyright (c) 2011-2023, NVIDIA CORPORATION.  All rights reserved. All
...
int32_t cbBeginSequence(void *ptr, const NvMediaParserSeqInfo *pnvsi)
{
    ...
    if(pnvsi->eCodec == NVMEDIA_VIDEO_CODEC_HEVC) {
        if((pnvsi->uCodedWidth < 144) || (pnvsi->uCodedHeight < 144)) {
            LOG_ERR("BeginSequence: (Width=%d, Height=%d) < (144, 144) NOT SUPPORTED for HEVC\n",
                    pnvsi->uCodedWidth, pnvsi->uCodedHeight);
                return -1;
            }
    }
    if(pnvsi->eCodec == NVMEDIA_VIDEO_CODEC_VP9) {
        if((pnvsi->uCodedWidth < 128) || (pnvsi->uCodedHeight < 128)) {
            LOG_ERR("BeginSequence: (Width=%d, Height=%d) < (128, 128) NOT SUPPORTED for VP9\n",
                    pnvsi->uCodedWidth, pnvsi->uCodedHeight);
                return -1;
            }
    }
...
NvMediaStatus cbDecodePicture(void *ptr, NvMediaParserPictureData *pd)
{
    ...
    if (pd->pCurrPic) {
        ...
        if (targetBuffer->videoSurface) {
            ...
            status = NvMediaIDEGetFrameDecodeStatus(ctx->decoder, 0, &frameStatus);
            if (status == NVMEDIA_STATUS_OK) {
...
)
{
    ...
    status = NvMediaIDEGetVersion(&version);
    if (status != NVMEDIA_STATUS_OK)
    {
        LOG_ERR("NVMEDIA_IDE_MAIN: NvMediaIDEGetVersion failed\n");
        return false;
    }
    if ((version.major != NVMEDIA_IDE_VERSION_MAJOR) || (version.minor != NVMEDIA_IDE_VERSION_MINOR))
    {
        LOG_ERR("NVMEDIA_IDE_MAIN: NvMediaIDEGetVersion version mismatch\n");
        return false;
    }
    LOG_INFO("NVMEDIA_IDE_MAIN: NvMediaIDE Major version - %d \n", NVMEDIA_IDE_VERSION_MAJOR);
    LOG_INFO("NVMEDIA_IDE_MAIN: NvMediaIDE Minor version - %d \n", NVMEDIA_IDE_VERSION_MINOR);
    ...
    for(i = 0; (i < ctx->loop) || (ctx->loop == -1); i++) {
        ...
        if(ctx->loop != 1 && !signal_stop) {
            if(ctx->stopDecoding) {
                ...
                if ((ctx->checkCRC == true) && (ctx->fpCrc))
                {
                    rewind(ctx->fpCrc);
                }
            }
...
static int Decode_orig(VideoDemoTestCtx *ctx)
{
    ...
    for(i = 0; (i < ctx->loop) || (ctx->loop == -1); i++) {
        ...
        rewind(ctx->file);
        if (NvMediaParserGetAttribute(ctx->parser, NvMParseAttr_GetBitstreamError, sizeof(uint32_t), &bitstream_error) == NVMEDIA_STATUS_OK)
        {
            LOG_DBG("Bitstream error after parsing %u\n", bitstream_error);
        }
