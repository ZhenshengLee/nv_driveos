...
)
{
    ...
    nvmstatus = NvMediaIJPDFillNvSciSyncAttrList(ijpdCtx, nvm_waiterAttrList, NVMEDIA_WAITER);
    if(status != NVMEDIA_STATUS_OK) {
       LOG_ERR("main: Failed to fill signaler attr list.\n");
       goto fail;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (status != NVMEDIA_STATUS_OK) {
        LOG_ERR("main: EOF NvSciSyncObj creation failed\n");
        goto fail;
    }
    LOG_DBG("main: Created EOF NvSciSyncObj\n");

    status = sAllocPreNvSciSyncObj(ijpdCtx, syncModule, &preSyncObj);
    if (status != NVMEDIA_STATUS_OK) {
    ...
    status = NvMediaIJPDRegisterNvSciSyncObj(ijpdCtx, NVMEDIA_PRESYNCOBJ, preSyncObj);
    if(status != NVMEDIA_STATUS_OK) {
    ...
    status = NvMediaIJPDRegisterNvSciBufObj(ijpdCtx, appBuffer.bufObj);
    if(status != NVMEDIA_STATUS_OK) {
    ...
    while(nextFrameFlag && (decodeStop == 0u)) {
        ...
        status = NvMediaIJPDInsertPreNvSciSyncFence(ijpdCtx, &appBuffer.preFence);
        if (status != NVMEDIA_STATUS_OK)
        {
...
fail:
    ...
    if (preSyncObj) {
        /* Unregister NvSciSyncObj */
        status = NvMediaIJPDUnregisterNvSciSyncObj(ijpdCtx, preSyncObj);
        if (status != NVMEDIA_STATUS_OK) {
            LOG_ERR("%s: Failed to unregister NvSciSyncObj\n", __func__);
        }
    ...
    if(appBuffer.bufObj) {
        ...
        status = NvMediaIJPDUnregisterNvSciBufObj(ijpdCtx, appBuffer.bufObj);
        if (status != NVMEDIA_STATUS_OK) {
