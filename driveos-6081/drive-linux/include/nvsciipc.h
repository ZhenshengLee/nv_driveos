 ...
 *
 * <b>Regarding fork() - QNX OS safety build</b>
 *
 ...
 * a asynchronouse error, i.e. failure on eDMA or PCIE. To get
...
/**
 * @brief Enable Intra-VM notification
 *
 * Control Intra-VM notification when connection or FIFO status is changed.
 * If notification is disabled, NvSciIpc doesn't send notification to remote
 * endpoint.
 * Intra-VM notification is enabled by default unless user disable it using
 * this API explicitly.
 ...
 * event blocking API (i.e. poll, NvSciIpcWaitEventQnx, WaitForEvent,
 * WaitForMultipleEvents etc.).
 * If connection recovery is required (i.e. remote endpoint is restarted)
 * after disabling notification, user shall call NvSciIpcGetEventSafe() till
 * it doesn't return NV_SCI_IPC_EVENT_CONN_RESET any more.
 *
 * @note Before calling this API to disable Intra-VM notification,
 ...
 *       If this is not guranteed, pending notification might be delivered
 *       from local to remote peer.
