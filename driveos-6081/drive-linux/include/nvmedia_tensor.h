...
/**
 * \brief Specifies that the tensor allocation is on CVSRAM.
 ...
 *                   @inputrange A valid, non-null pointer to \ref NvMediaTensor.
 *
 * @pre
 ...
 *                             \ref NvMediaTensorLockAccess.
 * \param[out] surfaceMap      A pointer to the surface descriptors.
 *                             @inputrange A valid, non-null pointer to \ref NvMediaTensorSurfaceMap.
 *
 ...
 * @retval NVMEDIA_STATUS_TIMEOUT if the wait on the tensor timed out.
 * @retval NVMEDIA_STATUS_ERROR if an error occurred.
 * @retval NVMEDIA_STATUS_INVALID_STATE if the API is triggered in the DRIVEOS
 *          state that is not allowed as per the *API Group*.
 * \ingroup lock_unlock
 *
 * @pre
 * - @a tensor must not be already locked
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *
 * @pre
 * - @a tensor must have been locked using NvMediaTensorLock()
 *
 *
 * @usage
 * - Allowed context for the API call
 ...
 *                    @inputrange A valid, non-null pointer to \ref NvMediaTensorTaskStatus.
 * \return \ref NvMediaStatus, the completion status of the operation:
 * @retval NVMEDIA_STATUS_OK                if the function is successful.
 * @retval NVMEDIA_STATUS_TIMEOUT           - if the wait on the tensor timed out.
 *                                          - if the task execution exceeded the user input value
 * @retval NVMEDIA_STATUS_OUT_OF_MEMORY     - if memory related DLA error occured
 * @retval NVMEDIA_STATUS_PFSD_ERROR        if a failure occurs during permanent fault diagnostics
 * @retval NVMEDIA_STATUS_BAD_PARAMETER     if any of the arguments are NULL or invalid.
 * @retval NVMEDIA_STATUS_ERROR             - if an internal DLA SW stack error occured.
 *                                          - if the input task status is not success, this error is
 *                                            returned for remaining tasks in the chain
 * @retval NVMEDIA_STATUS_INVALID_STATE     - if the API is triggered in the
 *                                            DRIVEOS state that is not allowed
 *                                            as per the *API Group*.
 *
 * @pre
 ...
 *                                 This should be a non-NULL value
 *
 * \return \ref NvMediaStatus, the completion status of the operation:
 * @retval NVMEDIA_STATUS_OK if the function is successful.
 * @retval NVMEDIA_STATUS_BAD_PARAMETER if any of the arguments are NULL or invalid.
 ...
 *
 * \param[out] version  A valid, non-NULL pointer to NvMediaVersion to
