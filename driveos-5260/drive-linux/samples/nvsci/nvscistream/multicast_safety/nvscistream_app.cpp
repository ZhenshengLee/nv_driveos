//
// NvSciStream Multicast Sample App.
//
...
int main(int argc, char *argv[])
{
            ...
            case 'p':
                ...
                if (nConsumer == 0U) {
                    ...
                    for (uint32_t i = 0; i < nConsumer; i++) {
                        strncpy(chname[i],
                                argv[PROD_CHANNEL_LIST_BASE_INDEX+i],
                                sizeof(chname[i]));
                    }
                }
                optcnt++;
    ...
    if (isConsumerChannel) {
        ...
        ConsumerChannel *consChannel =
            new ConsumerChannel(bufModule, syncModule, endpoint[0]);
        if (consChannel) {
            QueueType qType = (qtypeOpt == 1) ? QueueType::QueueType_FIFO
                                              : QueueType::QueueType_Mailbox;
        ...
        ProducerChannel *prodChannel =
            new ProducerChannel(bufModule, syncModule, endpoint, nConsumer);
        if (prodChannel) {
