...
namespace NvScistreamSample
{
    ...
    void ProducerChannel::createBlocks(void)
    {
        ...
        if (consumerCount > 1U) {
            ...
            LOG_DEBUG("[NvSciStream] Creating Multicast block!!!\n");
        }
        // Create pool
        ...
        for(uint32_t n=0U; n < consumerCount; n++) {
            NvSciStreamIpcSrcCreate(ipcEndpoint[n], syncModule, bufModule, &ipcSrcHandle[n]);
        }
    ...
    void ProducerChannel::connectToStream(void)
    {
        ...
        if (consumerCount == 1U) {
            LOG_DEBUG("[NvSciStream] Single Cast Case!!!\n");
            CHECK_NVSCIERR(NvSciStreamBlockConnect(producerHandle, ipcSrcHandle[0]));
        } else {
            LOG_DEBUG("[NvSciStream] Multi-Cast Case!!!\n");
