/*
 * Copyright (c) 2015-2022, NVIDIA CORPORATION.  All rights reserved. All
 ...
 */
typedef struct {
    /** \brief A list of all CDAC GPIO device node indices. */
    uint32_t idxs[DEVBLK_CDI_ROOT_DEVICE_MAX_GPIOS];
    /**
     * \brief Number of items in indices in gpio_idxs.
...
typedef struct {
     ...
     *
     * \param[in]  handle  A pointer to the device to use; Valid value: [non-NULL.
     * \param[in]  botEmbDataChunk  A pointer to the top sensor embedded data
     *                              \ref DevBlkCDIEmbeddedDataChunk structure.
     *                              Valid value: [non-NULL].
     ...
     * \retval NVMEDIA_STATUS_ERROR Authentication mismatch. (Failure)
 ...
 *
 *\note This function invokes the device driver function specified by the
 *  call to \ref ParseEmbedDataInfo().
 *
 * \param[in]  device               A pointer to the device to use;
 *                                  Valid value: [non-NULL].
 ...
 * \li \ref NVMEDIA_STATUS_ERROR authentication mismatch.
