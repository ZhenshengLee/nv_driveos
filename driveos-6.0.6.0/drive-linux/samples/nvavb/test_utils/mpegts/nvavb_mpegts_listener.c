...
void process_avb_data(NvmAvbSinkData *args, U8 *packet)
{
    ...
    {
        priv_data->fout = fopen("recv_mpegts.ts","w");
        U64 u64StreamId = NvAvtpGetStreamId(pHandle, (U8 *)packet);
        printf("Received stream id: %llx\n", u64StreamId);

        if(eAvtpSubHeaderType == eNvMpegts)
        {
            printf("IEC 61883-4 MPEGTS AVTP stream\n");
        }
        else
        {
            printf("This is NOT IEC 61883-4 MPEGTS AVTP stream!!, Skipping...\n");
            return;
        }
        NvAvtpGetMpegtsDataPayloadSize(pHandle, (U8 *)packet, &priv_data->payloadSize);
...
#ifdef DEBUG_DEPACK
     printf("%d non-1722 packages are skipped!!", num_non_1722_packages);
#endif
    close(fd_read_socket);
...
void breakLoop(int sig)
{
    fprintf(stderr, "Signal = %d, leaving the application!!\n", sig);
    loop_status = 0;
}
