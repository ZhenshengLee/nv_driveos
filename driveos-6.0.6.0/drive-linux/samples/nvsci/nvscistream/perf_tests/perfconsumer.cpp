    ...
    NvSciStreamBlock const upstreamBlock)
{
    ...
    if (testArg.testType == CrossProcCons) {
        ...
        CHECK_NVSCIERR(NvSciStreamIpcDstCreate2(ipcEndpoint,
                                                syncModule,
                                                bufModule,
                                                c2cPool,
                                                &ipcDst));
...
void PerfConsumer::handlePayload(void)
{
    ...
    if (testArg.latency) {
        presentEndTime2 = NvpGetTimeMark();
    }
    ...
    if (testArg.latency) {
        ...
        NVP_CHECKERR_EXIT(NvpRecordSample(&presentLatency,
                                          timerPtr[0],
                                          presentEndTime));
        NVP_CHECKERR_EXIT(NvpRecordSample(&presentLatency2,
                                          timerPtr[0],
                                          presentEndTime2));
