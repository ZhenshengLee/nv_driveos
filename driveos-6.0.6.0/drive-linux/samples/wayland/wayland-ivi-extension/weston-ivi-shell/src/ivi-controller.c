                             ...
                             uint32_t id)
{
    struct iviscreen *iviscrn = wl_resource_get_user_data(resource);
    struct screenshot_frame_listener *l;
    struct weston_compositor *compositor = iviscrn->output->compositor;
    (void)client;
    ...
    if(compositor->renderer->prepare_read_pixels) {
        compositor->renderer->prepare_read_pixels(iviscrn->output);
    }
...
destroy_screen(struct iviscreen *iviscrn)
{
    ...
    wl_resource_for_each_safe(resource, next, &iviscrn->resource_list) {
        ivi_wm_screen_send_screen_destroy(resource, iviscrn->id_screen);
...
get_config(struct weston_compositor *compositor, struct ivishell *shell)
{
	...
	// Set shell defaults if the config's default should be non zero
	// This is to cover if there is no config file or config file
	// without "ivi-shell" section in it
		       ...
		       int *argc, char *argv[])
{
    ...
    if ((shell->bkgnd_surface_id != -1) && shell->ivi_client_name) {
