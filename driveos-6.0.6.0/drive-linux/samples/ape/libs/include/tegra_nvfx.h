/*
 * tegra_nvfx.h - Shared NVFX interface between Tegra host driver and
...
#define NVFX_MEM_ALL_ACCESS		(NVFX_MEM_READ_ACCESS | NVFX_MEM_WRITE_ACCESS)

/* Memory mapped buffer. Needs to be unmapped when not used */
#define NVFX_MEM_MAPPED			0x4

/* Flag to indicate if buffer fill request (eg. DMA) is pending */
#define NVFX_MEM_REQ_PENDING	0x8
...
/**
 * nvfx_flush_params_t - Flushes input and cached data buffers
 *                       to support seek operations
 *
 * @call                nvfx_call_t parameters
 *
...
typedef struct {
	nvfx_call_params_t call;
	int32_t buffer[NVFX_MAX_CALL_PARAMS_WSIZE -
		(sizeof(nvfx_call_params_t) + sizeof(int32_t)) / sizeof(int32_t)];
} nvfx_param_i32_array_t;
