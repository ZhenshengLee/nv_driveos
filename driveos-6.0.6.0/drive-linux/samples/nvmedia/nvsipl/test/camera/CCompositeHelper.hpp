...
public:
    ...
    void SetRect(uint32_t group, bool soloInput, uint32_t outputWidth, uint32_t outputHeight) {
        ...
        if (!soloInput) {
            ...
            uint16_t yStep = outputHeight / (NUM_OF_ROWS/2);
            startX = outIndex * xStep;
            startY = (modIndex%2) * yStep;
            endX = startX + xStep;
            endY = startY + yStep;
        }
...
public:
    ...
    SIPLStatus Init(NvSiplRect *pRect,
        ...
        status = m_dispIf->Init(m_outWidth, m_outHeight, bufModule, *bufAttrList2d, soloInput);
        ...
        if (!soloInput) {
            // Create a blank NvSciBufObj of quarter display resolution to clear the parts of frame if any camera stream is lost
            status = CUtils::CreateRgbaBuffer(bufModule,
                                            *bufAttrList2d,
                                            m_outWidth/4,
                                            m_outHeight/2,
            ...
            if (m_blankFrame == nullptr) {
                LOG_ERR("CompositeHelper: Failed to create clear rectangle\n");
                return NVSIPL_STATUS_OUT_OF_MEMORY;
            }

            // Memset the NvSciBufObj (assumes the blank frame to be RGBA)
            const uint32_t buffSize = (m_outWidth/4) * (m_outHeight/2) * 4U;
            const uint32_t buffPitch = (m_outWidth/4) * 4U;
...
protected:
    ...
    SIPLStatus PostSync(T item, uint32_t row,
    ...
    // Helper to update the slot indices when multiple outputs are being used
    void UpdateIndexMultiOutput(uint32_t row) {
        switch (row) {
            case 0:
            case 1:
                m_uDstBufferIndex[row]++;
                if (m_uDstBufferIndex[row] > 2) {
                    m_uDstBufferIndex[row] = 0;
                }
                m_uDstBufferIndexNext[row]++;
                if (m_uDstBufferIndexNext[row] > 2) {
                    m_uDstBufferIndexNext[row] = 0;
                }
                break;
            case 2:
            default: // case 3
                m_uDstBufferIndex[row]++;
                if (m_uDstBufferIndex[row] > 5) {
                    m_uDstBufferIndex[row] = 3;
                }
                m_uDstBufferIndexNext[row]++;
                if (m_uDstBufferIndexNext[row] > 5) {
                    m_uDstBufferIndexNext[row] = 3;
                }
                break;
        }
    }
    ...
    void ThreadFunc() {
        ...
        while (m_bRunning) {
            ...
            uint32_t rowDisplay = 5;
            NvSciBufObj buffers[4] = {NULL}; // To pointers to buffers used by 2D in each while loop
            ...
            for (std::unique_ptr<CInputInfo<T>> &upInput : m_pGroupInfos[m_uGroupIndex].m_vInputs) {
                ...
                if (status != NVSIPL_STATUS_OK) {
                    if (!m_soloInput) {
                        LOG_DBG("Output unavailable, writing clear buffer in the relevant region\n");
                        itemEmpty = true;
                    } else {
                        continue;
                    }
                ...
                NvMediaStatus nvmStatus = NvMedia2DGetComposeParameters(m_p2dDevices[row].get(),
                                                                        &params);
                ...
                nvmStatus = NvMedia2DSetNvSciSyncObjforEOF(m_p2dDevices[row].get(),
                                                           params,
                                                           eofSyncObj);
                CHK_NVMSTATUS_AND_EXIT(nvmStatus, "NvMedia2DSetNvSciSyncObjforEOF");

                // Set the source layer parameters for layer zero
                nvmStatus = NvMedia2DSetSrcNvSciBufObj(m_p2dDevices[row].get(),
                                                       params,
                                                       0U,
                                                       srcBuf);
                ...
                if (rowDisplay != row) { // To keep track of rows instead of outputs
                    if (!m_soloInput) {
                        UpdateIndexMultiOutput(row);
                    } else {
                        if (((m_uDstBufferIndex[row] == 2) && (m_uDstBufferIndexNext[row] == 0)) ||
                            ((m_uDstBufferIndex[row] == 5) && (m_uDstBufferIndexNext[row] == 3))) {
                            m_uDstBufferIndex[row] = 0;
                            m_uDstBufferIndexNext[row] = 1;
                        } else {
                            uint32_t temp = m_uDstBufferIndex[row];
                            m_uDstBufferIndex[row] = m_uDstBufferIndexNext[row];
                            m_uDstBufferIndexNext[row] = temp;
                        }
                    }
                    rowDisplay = row;
                }

                buffers[row] = m_vDstBuffer[m_uDstBufferIndex[row]];
                LOG_DBG("row %d, current buffer idx %d, backup buffer idx %d, buffer being used %p\n",
                         row,
                         m_uDstBufferIndex[row],
                         m_uDstBufferIndexNext[row],
                         m_vDstBuffer[m_uDstBufferIndex[row]]);
                // Set the destination surface
                nvmStatus = NvMedia2DSetDstNvSciBufObj(m_p2dDevices[row].get(),
                                                       params,
                                                       m_vDstBuffer[m_uDstBufferIndex[row]]);
                CHK_NVMSTATUS_AND_EXIT(nvmStatus, "NvMedia2DSetDstNvSciBufObj for current buffer\n");
                if (!itemEmpty) {
                ...
                status = PostSync(item, row, &composeResult, false, false);
                if (status != NVSIPL_STATUS_OK) {
                   return;
                }
                if (!m_soloInput) {
                    // If multiple outputs are used, we need to ensure that
                    // the next buffer is filled with the same data as a backup buffer

                    // Acquire an empty NvMedia 2D parameters object
                    NvMedia2DComposeParameters paramsExtra;
                    nvmStatus = NvMedia2DGetComposeParameters(m_p2dDevices[row].get(),
                                                              &paramsExtra);
                    CHK_NVMSTATUS_AND_EXIT(nvmStatus, "NvMedia2DGetComposeParameters");
                    nvmStatus = NvMedia2DSetNvSciSyncObjforEOF(m_p2dDevices[row].get(),
                                                               paramsExtra,
                    ...
                    nvmStatus = NvMedia2DSetDstNvSciBufObj(m_p2dDevices[row].get(),
                                                           paramsExtra,
                                                           m_vDstBuffer[m_uDstBufferIndexNext[row]]);
                    CHK_NVMSTATUS_AND_EXIT(nvmStatus, "NvMedia2DSetDstNvSciBufObj for backup buffer\n");
                    if (!itemEmpty) {
                        status = PreSync(item, row, paramsExtra);
                        if (status != NVSIPL_STATUS_OK) {
                            return;
                        }
                    }
                    // Submit the compose operation
                    NvMedia2DComposeResult composeResultExtra;
                    nvmStatus = NvMedia2DCompose(m_p2dDevices[row].get(),
                                                 paramsExtra,
                                                 &composeResultExtra);
                    CHK_NVMSTATUS_AND_EXIT(nvmStatus, "NvMedia2DCompose");

                    status = PostSync(item, row, &composeResultExtra, !itemEmpty, true);
                    if (status != NVSIPL_STATUS_OK) {
                        return;
                    }
                }
            ...
            if (m_soloInput) {
                LOG_DBG("row %d, Buffer index %d, buffer %p\n",
                         rowDisplay,
                         m_uDstBufferIndex[rowDisplay],
                         m_vDstBuffer[m_uDstBufferIndex[rowDisplay]]);
                // Send composited output image to display
                SIPLStatus status = m_dispIf->WfdFlip(m_vDstBuffer[m_uDstBufferIndex[rowDisplay]]);
                if (status != NVSIPL_STATUS_OK) {
                    LOG_ERR("CompositeHelper: WfdFlip failed! status: %u\n", status);
                    return;
                }
            } else {
                uint32_t bufIdPipe0 = 4;
                uint32_t bufIdPipe1 = 4;
                if ((buffers[0] != NULL) && (buffers[1] != NULL)) {
                    if (buffers[0] == buffers[1]) {
                        bufIdPipe0 = 0;
                    } else {
                        LOG_ERR("Buffer mismatch\n");
                        return;
                    }
                } else if (buffers[0] != NULL) {
                    bufIdPipe0 = 0;
                } else if (buffers[1] != NULL) {
                    bufIdPipe0 = 1;
                }
                if (bufIdPipe0 != 4) {
                    LOG_DBG("Flip buffer pipe0 %p\n",buffers[bufIdPipe0]);
                    // Send composited output image to display pipeline 1
                    SIPLStatus status = m_dispIf->WfdFlip(buffers[bufIdPipe0]);
                    if (status != NVSIPL_STATUS_OK) {
                        LOG_ERR("CompositeHelper: WfdFlip failed! status: %u\n", status);
                        return;
                    }
                    ...
                    // Dummy flip since we cant use Pipeline 2 without pipeline 1 in Linux
                    SIPLStatus status = m_dispIf->WfdFlip(m_vDstBuffer[m_uDstBufferIndex[0]]);
                    if (status != NVSIPL_STATUS_OK) {
                        LOG_ERR("CompositeHelper: WfdFlip failed! status: %u\n", status);
                        return;
                    }
...
#endif
                if ((buffers[2] != NULL) && (buffers[3] != NULL)) {
                ...
                } else if (buffers[2] != NULL) {
                    bufIdPipe1 = 2;
                } else if (buffers[3] != NULL) {
                    bufIdPipe1 = 3;
                }
                if (bufIdPipe1 != 4) {
                    LOG_DBG("Flip buffer pipe1 %p\n",buffers[bufIdPipe1]);
                    // Send composited output image to display pipeline 2
                    SIPLStatus status = m_dispIf->WfdFlip(buffers[bufIdPipe1]);
                    if (status != NVSIPL_STATUS_OK) {
                        LOG_ERR("CompositeHelper: WfdFlip failed! status: %u\n", status);
                        return;
                    }
    ...
    // Output
    uint32_t m_uDstBufferIndex[NUM_OF_ROWS] = {2, 2, 5, 5};
    uint32_t m_uDstBufferIndexNext[NUM_OF_ROWS] = {0, 0, 3, 3};
