...
class CCmdLineParser
{
 public:
    ...
    static void ShowUsage(void)
    {
        ...
        cout << "--disableSubframe                          :Disable Subframe feature\n";
        ...
        cout << "--syncGPIOIdxDes 'gpios'                   :GPIO index of the deserializer for FRSYNC signal. Applicable only for the specific board\n";
        ...
        cout << "--enableAuth                               :Forcefully enable authentication for all camera sensors\n";
        cout << "                                           :If not specified - authentication is controlled by Platform Config database\n";
#if !NV_IS_SAFETY
        cout << "--syncMuxSelDes 'Indexes'                  :Selection index to the multiplexer. Applicable only for the specific board\n";
    ...
    {
        const char *const short_options = "hc:m:nr:v:t:lR012sMP:aCk:N:O:eIoLi:Sf:K:W:T:D:U:b"
#ifdef NVMEDIA_QNX
                                          "7"
#endif //NVMEDIA_QNX
#if !NV_IS_SAFETY
    ...
    void PrintArgs() const
    {
        ...
        if (bEnableAuth) {
            cout << "Authentication: enabled" << endl;
        } else {
            cout << "Authentication: controlled by Platform Config" << endl;
        }
#if !NV_IS_SAFETY
        if (uNumDisplays > 0u) {
