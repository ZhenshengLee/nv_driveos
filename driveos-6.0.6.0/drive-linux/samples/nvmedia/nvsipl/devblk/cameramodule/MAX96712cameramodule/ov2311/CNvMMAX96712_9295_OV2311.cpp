...
SIPLStatus CNvMMAX96712_9295_OV2311::CheckModuleStatus()
{
    ...
    if (status != NVSIPL_STATUS_OK) {
        SIPL_LOG_ERR_STR_INT("SetPower failed with SIPL error %d", status);
        return status;
    }
    ...
    linkAction.eAction = CNvMDeserializer::LinkAction::LINK_ENABLE;
    ...
    if (mediaStatus != NVMEDIA_STATUS_OK) {
        ...
        if (statusPwr != NVSIPL_STATUS_OK) {
            SIPL_LOG_ERR_STR_INT("SetPower failed with SIPL error %d", statusPwr);
            return statusPwr;
        }
    }
