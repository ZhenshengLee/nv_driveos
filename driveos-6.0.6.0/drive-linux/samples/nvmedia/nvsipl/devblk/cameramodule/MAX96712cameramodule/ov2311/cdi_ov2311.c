    ...
    const size_t parsedInfoStructSize)
{
    ...
    /*error information */
    parsedInfo->errorFlag = 0; // no error info but this would be how it is indicated
done:
    return status;
}
    ...
    uint32_t *exposureVal)
{
    ...
    exposureVal[0] = (exposureControl->exposureTime[0] * drvHandle->tclk + drvHandle->hts/2)/drvHandle->hts;
    if(exposureVal[0]<2) exposureVal[0] = 0x01;
    ...
    float *dgain)
{
    ...
    /* Coarse Step */
    for(n=0;n<3;n++){
        if(realgain < (base[n+1]*(1.0f+1.0f/OV2311_AGAIN_SCALES))) break;
    }

    /* Fine Step */
    for(i=0;i<(OV2311_AGAIN_SCALES-1);i++){
        if(realgain <= (base[n]*(1.0f+1.0f*i/OV2311_AGAIN_SCALES))) break;
    }
    ...
    const DevBlkCDIExposure *exposureControl)
{
    ...
    if (NVMEDIA_TRUE == exposureCtrlSettings->expTimeValid) {
        ...
        if(drvHandle->vts-7 > exposureRegv){
            exposureRegv = drvHandle->vts-7-exposureRegv;
        }else{
            exposureRegv = 0;
        }
        regv16 = exposureRegv & 0xFFFFu;
    ...
    const size_t sensrCtrlStructSize)
{
    ...
    if (sensorControl->numSensorContexts == 1) {
        ...
        SIPL_LOG_ERR_STR_INT("OV2311: Exceeded maximum number of sensor contexts (max 1)",
                (int32_t)sensorControl->numSensorContexts);
        status = NVMEDIA_STATUS_NOT_SUPPORTED;
    }
    ...
    const size_t sensorAttrStructSize)
{
    ...
    maxVal = ((float)((drvHandle->vts -12) * drvHandle->hts)) / ((float) drvHandle->tclk);
