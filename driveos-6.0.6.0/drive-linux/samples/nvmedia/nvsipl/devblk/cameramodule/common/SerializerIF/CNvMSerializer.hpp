...
public:
     ...
     */
    virtual SIPLStatus SetConfig(const SerInfo *const serializerInfo, DeviceParams *const params) = 0;
     ...
     * @retval      (SIPLStatus) error status propagated
     */
    virtual SIPLStatus GetErrorInfo(std::uint8_t * const buffer, const std::size_t bufferSize, std::size_t &size) const = 0;
