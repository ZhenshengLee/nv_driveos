    ...
    DevBlkCDIDevice *handle)
{
    ...
    _DriverHandle *drvHandle = (_DriverHandle *)handle->deviceDriverHandle;
    DevBlkCDII2CReg rx_port_ctrl = {0x0C, 0x0F};
    ...
    I2CTranslateDS90UB9724 i2cTransID)
{
    _DriverHandle *drvHandle = (_DriverHandle *)handle->deviceDriverHandle;
    NvMediaStatus status = NVMEDIA_STATUS_OK;
    DevBlkCDII2CReg i2cPassThrough = {0x58, 0x5E};
    ...
    if (i2cTransID == CDI_DS90UB9724_I2C_TRANSLATE_SER) {
        ...
        serI2C_regs[1].data = (slaveID << 1) | (lock ? 1 : 0); // lock the ser address?
        serI2C_regs[2].data = (slaveAlias << 1) | (lock ? 1 : 0); // Enable auto ack for now?
    ...
    const uint32_t max_iters)
{
    ...
    for (uint32_t i = 0U; i < max_iters; i++) {
        ...
        const uint16_t rx_port = ((val >> 6U) & 0x3U);
        if (link != rx_port) {
       ...
       uint8_t link)
{
    ...
    DevBlkCDII2CReg AEQ_LMS_regs[] = {
        ...
        {0x01, 0x21, 20*1000}, // Soft reset, wait 20ms
        {0xB1, 0x90},
        {0xB2, 0x40},
    };
    ...
    uint8_t link)
{
    ...
    DevBlkCDII2CReg linkSpeedFPDLink3_regs[] = {
        {0xB0, (link+1)<<2}, // Select the link
        {0x4C, 1<<(link)|(link<<4)}, // Configure write and read for the correct link
        ...
        {0x0C, enabledLinks | (link+1)}, // Enable RX port based on link
    };
    ...
    return DevBlkCDII2CPgmrWriteArray(drvHandle->i2cProgrammer, &linkSpeedFPDLink3);
}

static NvMediaStatus
SetLinkSpeedFPDLink4(
    DevBlkCDIDevice *handle,
    uint8_t link)
{
    _DriverHandle *drvHandle = (_DriverHandle *)handle->deviceDriverHandle;
    ...
    DevBlkCDII2CReg linkSpeedFPDLink4_regs[] = {
        {0xB0, (link+1)<<2}, // Select the link
        ...
        {0x58, 0x16}, // Backchannel: forward decoded aliases, 50mbps (953 backward comp)
        {0xB1, 0x04},
    ...
    void *parameter)
{
    ...
    switch (parameterType) {
        case CDI_WRITE_PARAM_CMD_DS90UB9724_SET_I2C_TRANSLATION:
            ...
            break;
        case CDI_WRITE_PARAM_CMD_DS90UB9724_CHECK_LOCK:
            if (parameterSize == sizeof(param->link)) {
                isValidSize = true;
                status = checkLinkLock(handle,
                                        param->link);
            }
            break;
        ...
        case CDI_WRITE_PARAM_CMD_DS90UB9724_ENABLE_AEQ_LMS:
            ...
            break;
        case CDI_WRITE_PARAM_CMD_DS90UB9724_SET_LINK_SPEED_FPDLINK3:
            if (parameterSize == sizeof(param->link)) {
                isValidSize = true;
                status = SetLinkSpeedFPDLink3(handle,
                                              param->link);
            }
            break;
        case CDI_WRITE_PARAM_CMD_DS90UB9724_SET_LINK_SPEED_FPDLINK4:
            if (parameterSize == sizeof(param->link)) {
                isValidSize = true;
                status = SetLinkSpeedFPDLink4(handle,
                                              param->link);
            }
            break;
