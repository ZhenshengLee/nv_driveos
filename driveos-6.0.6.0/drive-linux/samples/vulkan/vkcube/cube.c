...
static char *tex_files[] = {"../vulkan_resources/lunarg.ppm"};
...
#define APP_NAME_STR_LEN 80
    ...
    PFN_vkGetPhysicalDeviceSurfaceCapabilities2EXT fpGetPhysicalDeviceSurfaceCapabilities2EXT;
    PFN_vkRegisterDisplayEventEXT fpRegisterDisplayEventEXT;
    PFN_vkGetSwapchainCounterEXT fpGetSwapchainCounterEXT;
    PFN_vkRegisterDeviceEventEXT fpRegisterDeviceEventEXT;
    PFN_vkDisplayPowerControlEXT fpDisplayPowerControlEXT;
    VkDisplayKHR vkDisplay;
    bool get_vblank_count;
    bool wait_vblank;
    bool test_display_power_control;
    bool detect_hotplug;
#endif
};
...
int create_ivi_surface(struct demo *demo);
#endif
...
#if defined(VK_USE_PLATFORM_DISPLAY_KHR)
    if (demo->wait_vblank) {
        VkDisplayEventInfoEXT eventInfo = {
        ...
        VkFence vsyncFence;
        err = demo->fpRegisterDisplayEventEXT(demo->device, demo->vkDisplay, &eventInfo, NULL, &vsyncFence);
        assert(!err);

        err = vkWaitForFences(demo->device, 1, &vsyncFence, VK_TRUE, 50000000);

        vkDestroyFence(demo->device, vsyncFence, NULL);
    }

    if (demo->get_vblank_count) {
        uint64_t counter;
        err = demo->fpGetSwapchainCounterEXT(demo->device,
                                             demo->swapchain,
                                             VK_SURFACE_COUNTER_VBLANK_EXT,
                                             &counter);
        assert(!err);

        printf("Swapchain vblank counter: %lu\n", counter);
    }
#endif
}
...
static void demo_prepare_buffers(struct demo *demo) {
    ...
    if (demo->get_vblank_count) {
        ...
        err = demo->fpGetPhysicalDeviceSurfaceCapabilities2EXT(demo->gpu, demo->surface, &caps);
        assert(!err);

        if (!(caps.supportedSurfaceCounters & VK_SURFACE_COUNTER_VBLANK_EXT)) {
            printf("VK_SURFACE_COUNTER_VBLANK_EXT not supported\n");
            exit(1);
        } else {
            printf("VK_SURFACE_COUNTER_VBLANK_EXT supported\n");
        }
    }
#endif
    ...
    if (surfCapabilities.currentExtent.width == 0xFFFFFFFF) {
        ...
        // Adjust the projection matrix for the new aspect ratio
        float aspectRatio = ((float) demo->width/ (float) demo->height);
        mat4x4_perspective(demo->projection_matrix, (float)degreesToRadians(45.0f), aspectRatio, 0.1f, 100.0f);
        // Flip projection matrix from GL to Vulkan orientation.
        demo->projection_matrix[1][1] *= -1;
    }
...
#include "lunarg.ppm.h"
    ...
    rgba_data += (layout->rowPitch * (*height));
...
static const struct wl_shell_surface_listener shell_surface_listener UNUSED = {handle_ping, handle_configure, handle_popup_done};
...
static void demo_create_window(struct demo *demo) {
    ...
    if (demo->shell) {
        ...
        if (demo->ivi_application) {
            demo->ivi_surfaceId = DEFAULT_IVI_SURFACE_ID;
            if (!create_ivi_surface(demo)) {
...
static void demo_run_display(struct demo *demo) {
    ...
    if (demo->test_display_power_control) {
        // turn the display on and off to test vkDisplayPowerControlEXT
        printf("Testing vkDisplayPowerControlEXT\n");
        printf("Turning display off\n");
        VkDisplayPowerInfoEXT powerOnInfo = {VK_STRUCTURE_TYPE_DISPLAY_POWER_INFO_EXT, NULL, VK_DISPLAY_POWER_STATE_ON_EXT};
        VkDisplayPowerInfoEXT powerOffInfo = {VK_STRUCTURE_TYPE_DISPLAY_POWER_INFO_EXT, NULL, VK_DISPLAY_POWER_STATE_OFF_EXT};
        demo->fpDisplayPowerControlEXT(demo->device, demo->vkDisplay, &powerOffInfo);
        sleep(5);
        printf("Turning display back on\n");
        demo->fpDisplayPowerControlEXT(demo->device, demo->vkDisplay, &powerOnInfo);
        sleep(5);
        printf("Finished testing vkDisplayPowerControlEXT\n");
    }

// XXX: Currently only linux supports VK_EXT_display_control
#if defined(__linux__)
    VkFence hotplugFence;
    ...
    if (demo->detect_hotplug) {
        demo->fpRegisterDeviceEventEXT(demo->device, &deviceEventInfo, NULL, &hotplugFence);
    }
#endif
...
#if defined(__linux__)
        if (demo->detect_hotplug) {
            VkResult res = vkWaitForFences(demo->device, 1, &hotplugFence, VK_TRUE, 0);
            if (res == VK_SUCCESS) {
                printf("Hotplug detected, requerying displays to verify if we have one connected that we want to use\n");

                // Register fence for another hotplug event
                demo->fpRegisterDeviceEventEXT(demo->device, &deviceEventInfo, NULL, &hotplugFence);

                // First, destroy old surface
                vkDestroySurfaceKHR(demo->inst, demo->surface, NULL);

                // Check for displays
                uint32_t display_count = 0;
                res = vkGetPhysicalDeviceDisplayPropertiesKHR(demo->gpu, &display_count, NULL);
                assert(!res);
                if (display_count == 0) {
                    printf("No displays detected, waiting for another hotplug\n");
                    res = vkWaitForFences(demo->device, 1, &hotplugFence, VK_TRUE, UINT64_MAX);
                    assert(!res);
                    if (res == VK_SUCCESS) {
                        // Register fence for another hotplug event
                        demo->fpRegisterDeviceEventEXT(demo->device, &deviceEventInfo, NULL, &hotplugFence);
                ...
                demo_create_display_surface(demo);
                // Try to create a new swapchain
                demo_resize(demo);
            }
        }
#endif
...
#if defined(__linux__)
    if (demo->detect_hotplug) {
        vkDestroyFence(demo->device, hotplugFence, NULL);
    }
#endif
}
#endif
...
static void demo_init_vk(struct demo *demo) {
...
#if defined(VK_USE_PLATFORM_DISPLAY_KHR)
    VkBool32 displaySurfaceCounterExtFound = 0;
#endif
    ...
    if (instance_extension_count > 0) {
        ...
        for (uint32_t i = 0; i < instance_extension_count; i++) {
            ...
            if (!strcmp(VK_EXT_DISPLAY_SURFACE_COUNTER_EXTENSION_NAME, instance_extensions[i].extensionName)) {
                displaySurfaceCounterExtFound = 1;
                demo->extension_names[demo->enabled_extension_count++] = VK_EXT_DISPLAY_SURFACE_COUNTER_EXTENSION_NAME;
            }
#elif defined(VK_USE_PLATFORM_ANDROID_KHR)
...
#if defined(VK_USE_PLATFORM_DISPLAY_KHR)
    if (demo->get_vblank_count && !displaySurfaceCounterExtFound) {
        ERR_EXIT("vkEnumerateInstanceExtensionProperties failed to find "
                 "the " VK_EXT_DISPLAY_SURFACE_COUNTER_EXTENSION_NAME
    ...
    demo->enabled_extension_count = 0;
#if defined(VK_USE_PLATFORM_DISPLAY_KHR)
    VkBool32 displayControlExtFound = 0;
#endif
    memset(demo->extension_names, 0, sizeof(demo->extension_names));
...
#if defined(VK_USE_PLATFORM_DISPLAY_KHR)
            else if (!strcmp(VK_EXT_DISPLAY_CONTROL_EXTENSION_NAME, device_extensions[i].extensionName)) {
                displayControlExtFound = 1;
                demo->extension_names[demo->enabled_extension_count++] = VK_EXT_DISPLAY_CONTROL_EXTENSION_NAME;
            }
#endif
            assert(demo->enabled_extension_count < 64);
        }
...
#if defined(VK_USE_PLATFORM_DISPLAY_KHR)
    if ((demo->wait_vblank ||
         demo->get_vblank_count ||
         demo->test_display_power_control ||
         demo->detect_hotplug) &&
         !displayControlExtFound) {
        ERR_EXIT("vkEnumerateDeviceExtensionProperties failed to find "
                 "the " VK_EXT_DISPLAY_CONTROL_EXTENSION_NAME
                 " extension.\n\nDo you have a compatible "
                 "Vulkan installable client driver (ICD) installed?\nPlease "
                 "look at the Getting Started guide for additional "
                 "information.\n",
...
#if defined(VK_USE_PLATFORM_DISPLAY_KHR)
    if (demo->get_vblank_count) {
        GET_INSTANCE_PROC_ADDR(demo->inst, GetPhysicalDeviceSurfaceCapabilities2EXT);
    }
#endif
}
...
// Create a WSI surface for the window:
    ...
    if (demo->test_display_power_control) {
        GET_DEVICE_PROC_ADDR(demo->device, DisplayPowerControlEXT);
    }
    // XXX: Currently only linux supports VK_EXT_display_control
#if defined(__linux__)
    if (demo->detect_hotplug) {
        GET_DEVICE_PROC_ADDR(demo->device, RegisterDeviceEventEXT);
    }
#endif
#endif
...
static void registry_handle_global(void *data, struct wl_registry *registry, uint32_t id, const char *interface,
    ...
    if (strcmp(interface, "wl_compositor") == 0) {
        ...
        demo->ivi_application = (struct ivi_application *)wl_registry_bind(registry, id, &ivi_application_interface, 1);
        // By default use hmi controller
        demo->has_ivi_controller = 0;
...
int create_ivi_surface(struct demo *demo)
{
    uint32_t id_ivisurf = demo->ivi_surfaceId + (uint32_t)getpid();
    demo->ivi_surface =
        ivi_application_surface_create(demo->ivi_application, id_ivisurf, demo->window);

    if (demo->ivi_surface == NULL) {
        printf("Failed to create ivi_client_surface\n");
        return 0;
    }

    ivi_surface_add_listener(demo->ivi_surface, &ivi_surface_listener, demo);
    return 1;
}
#endif
...
static void demo_init(struct demo *demo, int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
...
#if defined(VK_USE_PLATFORM_DISPLAY_KHR)
        if (strcmp(argv[i], "--wait-vblank") == 0) {
            demo->wait_vblank = true;
            continue;
        }
        if (strcmp(argv[i], "--vblank-count") == 0) {
            demo->get_vblank_count = true;
            continue;
        }
        if (strcmp(argv[i], "--display-power") == 0) {
            demo->test_display_power_control = true;
            continue;
        }
        if (strcmp(argv[i], "--detect-hotplug") == 0) {
            demo->detect_hotplug = true;
            continue;
        }
#endif
        ...
        fprintf(stderr,
                ...
                "\t[--wait-vblank] [--vblank-count] [--display-power] [--detect-hotplug]\n",
