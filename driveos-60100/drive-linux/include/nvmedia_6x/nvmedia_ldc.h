...
///
#define NVMEDIA_LDC_VERSION_MAJOR 11
...
///    argument passed to #NvMediaLdcSetGeometry(), and less than or equal to
///    16384.
...
typedef struct
{
    ...
    /// @brief Number of control points in the controlPoints array.
    ///
...
///
NvMediaStatus
NvMediaLdcDestroy(NvMediaLdc const * const handle);
...
/// The object will be initialized and ready to use.
...
///
/// The handle returned in @a params is tied to the specific NvMediaLdc context
...
///
NvMediaStatus
NvMediaLdcCreateParameters(NvMediaLdc const * const handle,
...
///
NvMediaStatus
NvMediaLdcDestroyParameters(NvMediaLdc const * const handle, NvMediaLdcParameters const params);
...
///
NvMediaStatus
NvMediaLdcSetFilter(NvMediaLdc const * const handle,
                    NvMediaLdcParameters const params,
...
///
NvMediaStatus
NvMediaLdcSetGeometry(NvMediaLdc const * const handle,
                      NvMediaLdcParameters const params,
...
///
NvMediaStatus
NvMediaLdcSetIptParameters(NvMediaLdc const * const handle,
                           NvMediaLdcParameters const params,
                           NvMediaLdcIptParameters const *const iptParams);

/// @brief Sets parameters for LDC Warp Map.
///
/// This enables VIC geotrans processing in Warp Map mode. The map data accessed
/// through warpMapParams is copied to NvMediaLdc internal data structures.
///
...
///
NvMediaStatus
NvMediaLdcSetWarpMapParameters(NvMediaLdc const * const handle,
                               NvMediaLdcParameters const params,
...
/// or Warp Map mode is enabled. The map data accessed through maskMapParams is
/// copied to NvMediaLdc internal data structures.
///
...
///
NvMediaStatus
NvMediaLdcSetMaskMapParameters(NvMediaLdc const * const handle,
                               NvMediaLdcParameters const params,
...
///
NvMediaStatus
NvMediaLdcSetTnrParameters(NvMediaLdc const * const handle,
                           NvMediaLdcParameters const params,
...
///
NvMediaStatus
NvMediaLdcResetTnr(NvMediaLdc const * const handle, NvMediaLdcParameters const params);
...
///
NvMediaStatus
NvMediaLdcSetChecksumMode(NvMediaLdc const * const handle,
                          NvMediaLdcParameters const params,
...
///
NvMediaStatus
NvMediaLdcGetChecksum(NvMediaLdc const * const handle,
...
/// - All corners must be aligned to a a multiple of 2.
/// - If IPT and warp map processing are both disabled, must equal the
...
/// - Width must be at least 64 and height at least 16.
/// - All corners must be aligned to a a multiple of 2.
/// - If IPT and warp map processing are both disabled, must equal the
///   dimensions of the source image.
///
...
/// - The hardware accelerator utilized by NvMedia LDC accesses the surfaces in
///   64x16 (width x height) pixel tiles. Using surface sizes and rectangle
///   dimensions that are exact multiples of the tile size most optimally aligns
///   with this hardware access pattern.
/// - In IPT and warp map mode, using control point log2 interval of at least 2
///   provides the best performance.
///
/// The result info returned in @a result is tied to the specific NvMediaLdc
...
///
NvMediaStatus
NvMediaLdcProcess(NvMediaLdc const * const handle,
                  NvMediaLdcParameters const params,
...
//
// Version 9.0.0 September 8, 2023
// - Reject odd rectangle coordinates regardless of surface format
// - Reject destination rectangles smaller than 64x16 pixels
//
// Version 10.0.0 October 11, 2023
// - Add restrictions on xSobel output surfaces
//
// Version 11.0.0 December 1, 2023
// - Add restrictions on source and destination rectangles when IPT and warp map
//   processing are both disabled
//
