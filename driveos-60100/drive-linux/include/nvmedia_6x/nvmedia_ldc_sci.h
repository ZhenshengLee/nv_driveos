...
///
NvMediaStatus
NvMediaLdcFillNvSciBufAttrList(NvMediaLdc const * const handle, NvSciBufAttrList const attrList);
...
/// NvMediaLdc context instance passed in @a handle. NvSciSyncObjs created using
/// such attribute values must not be used after the NvMediaLdc context instance
/// has been destroyed.
///
...
///
NvMediaStatus
NvMediaLdcFillNvSciSyncAttrList(NvMediaLdc const * const handle,
...
///
/// Every NvSciBufObj used by NvMedia LDC must be registered by a call to this
...
///
NvMediaStatus
NvMediaLdcRegisterNvSciBufObj(NvMediaLdc const * const handle, NvSciBufObj const bufObj);

/// @brief Register an #NvSciSyncObj with NvMedia LDC.
///
/// Every NvSciSyncObj used by NvMedia LDC must be registered by a call to this
/// function before it is used. Also the NvSciSyncObjs associated with any
/// NvSciSyncFences passed to NvMediaLdcInsertPreNvSciSyncFence() must be
/// registered.
///
/// @param[in] handle       Pointer to the NvMediaLdc context.
...
///
NvMediaStatus
NvMediaLdcRegisterNvSciSyncObj(NvMediaLdc const * const handle,
...
///
NvMediaStatus
NvMediaLdcUnregisterNvSciBufObj(NvMediaLdc const * const handle, NvSciBufObj const bufObj);
...
/// NvMediaLdcProcess() operation, an error will be returned.
///
/// @param[in] handle   Pointer to the NvMediaLdc context.
...
///
NvMediaStatus
NvMediaLdcUnregisterNvSciSyncObj(NvMediaLdc const * const handle, NvSciSyncObj const syncObj);
...
///                                       with NvMedia LDC, or one of the
...
/// - @a syncObj must be valid NvSciSyncObj handle.
/// - @a syncObj must have been previously registered with
///   NvMediaLdcRegisterNvSciSyncObj() using an EOF event type.
///
/// @usage
/// - Allowed context for the API call
///   - Interrupt handler: No
...
///
NvMediaStatus
NvMediaLdcSetNvSciSyncObjforEOF(NvMediaLdc const * const handle,
...
///   registered with NvMediaLdcRegisterNvSciSyncObj() using a PRESYNC event
///   type.
///
/// @usage
/// - Allowed context for the API call
...
///
NvMediaStatus
NvMediaLdcInsertPreNvSciSyncFence(NvMediaLdc const * const handle,
                                  NvMediaLdcParameters const params,
...
/// NvMediaLdcProcess() operation can be queried using this function only until
/// the next operation using the same NvSciSyncObj for the EOF event is
/// submitted. When more operations are submitted, the NvMediaLdcResult structs
/// for the previous operations are no longer considered valid by this function.
///
/// The fence returned in @a syncFence is tied to the specific NvMediaLdc
/// context instance passed in @a handle and must not be used after the
/// NvMediaLdc context instance has been destroyed.
///
/// @param[in]  handle     Pointer to the NvMediaLdc context.
...
///   configured with NvMediaLdcSetNvSciSyncObjforEOF().
///
/// @usage
/// - Allowed context for the API call
///   - Interrupt handler: No
...
///
NvMediaStatus
NvMediaLdcGetEOFNvSciSyncFence(NvMediaLdc const * const handle,
...
///                                       - @a surface or its duplicate was not
///                                         registered.
/// @retval NVMEDIA_STATUS_ERROR          An internal failure occurred.
///
/// @pre
...
/// - @a params must be valid NvMediaLdcParameters handle created with
///   NvMediaLdcCreateParameters().
/// - @a surface must be valid NvSciBufObj handle.
/// - @a surface must have been previously registered with
///   NvMediaLdcRegisterNvSciBufObj().
///
/// @usage
/// - Allowed context for the API call
...
///
NvMediaStatus
NvMediaLdcSetSrcSurface(NvMediaLdc const * const handle,
                        NvMediaLdcParameters const params,
...
///
NvMediaStatus
NvMediaLdcSetDstSurface(NvMediaLdc const * const handle,
                        NvMediaLdcParameters const params,
                        NvSciBufObj const surface);
...
///
NvMediaStatus
NvMediaLdcSetPreviousSurface(NvMediaLdc const * const handle,
                             NvMediaLdcParameters const params,
                             NvSciBufObj const surface);

/// @brief Sets the surface for xSobel output.
///
/// The surface must be a single-plane surface with format compatible with the
/// first plane of the source surface. The surface dimensions must equal the
/// destination rectangle dimensions.
///
/// The xSobel output can only be used if warp map processing has been enabled
/// with NvMediaLdcSetWarpMapParameters() or IPT processing has been enabled
/// with NvMediaLdcSetIptParameters(), and TNR processing has NOT been enabled
...
///
NvMediaStatus
NvMediaLdcSetXSobelDstSurface(NvMediaLdc const * const handle,
                              NvMediaLdcParameters const params,
                              NvSciBufObj const surface);
...
/// The surface must be a single-plane surface with format compatible with the
/// first plane of the source surface. The surface dimensions must equal the
/// destination rectangle dimensions rounded up to the nearest multiple of 4,
/// and then divided by 4. For example, if the destination rectangle is 640x478
/// pixels, the 4x4 downsampled xSobel surface must be 160x120 pixels.
///
/// The 4x4 downsampled xSobel output can only be used if also full-size xSobel
/// output has been enabled with NvMediaLdcSetXSobelDstSurface().
///
/// The xSobel output can only be used if warp map processing has been enabled
/// with NvMediaLdcSetWarpMapParameters() or IPT processing has been enabled
/// with NvMediaLdcSetIptParameters(), and TNR processing has NOT been enabled
...
///
NvMediaStatus
NvMediaLdcSetDownsampledXSobelDstSurface(NvMediaLdc const * const handle,
                                         NvMediaLdcParameters const params,
                                         NvSciBufObj const surface);
