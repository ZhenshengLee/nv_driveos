...
typedef enum {
    ...
    /** \hideinitializer \brief passing region of interest parameters */
    NvMediaVideoEncFrame_ROIParams           = (1<<4),
...
typedef struct {
    ...
    /** Region of interest parameters. They are valid when \ref EncodeParamsFlag set with
      * \ref NvMediaVideoEncFrame_ROIParams */
    /** Num Of ROI Regions */
...
typedef enum {
    ...
    /** This attribute is used to pass HW config files as specified in \ref
        NvVideoEncodeCfgFiles for current encoding session.
        This is only required for HW validation. This attribute must be set
        before first frame of encoding. */
 ...
 * These is not required for actual Usecase support.
 */
typedef struct
{
