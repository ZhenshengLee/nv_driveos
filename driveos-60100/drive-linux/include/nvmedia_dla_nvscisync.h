 ...
 *                          an %NvMediaDla signaler or an %NvMediaDla waiter or
 *                          an %NvMediaDla signaler-waiter.
 ...
 *
 * @pre
 *  - @a attrlist must be a valid \ref NvSciSyncAttrList.
 *  - This function must be called before allocating the
 * NvSciSyncObj
 *
 * @usage
 * - Allowed context for the API call
 ...
 * NvSciSyncAttrKey_RequireDeterministicFences set to
 * - TRUE
 *
 * The application must not set these attributes in the NvSciSyncAttrList passed
 * as an input to this function.
 ...
 *
 * @pre
 *  - @a nvscisync to be registered must have been created
 * with the NvSciSyncAttrList returned by NvMedia-DLA.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 * this @a scisyncobj has completed.
 * If the function is called while @a scisyncobj is still in use by any
 * NvMediaDla operations, the behavior is undefined.
 *
 * \param[in] dla         An NvMedia DLA device handle.
 *                        @inputrange A non-null pointer to an \ref NvMediaDla
 *                        created with NvMediaDlaCreate().
 * \param[in] scisyncobj  An \ref NvSciSyncObj to be unregistered with @a dla.
 *                        @inputrange A non-null pointer created by
 *                        NvSciSyncObjAlloc().
 *
 * \return \ref NvMediaStatus, the completion status of the operation:
 ...
 *
 * @pre
 *  - @a scisyncobj have been created and registered with
 * the input NvMediaDLA handle.
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *
 * @pre
 *  - @a nvscisyncSOF must have been created and registered with
 * the input NvMediaDLA handle using NvMediaDlaRegisterNvSciSyncObj().
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *
 * @pre
 *  - @a nvscisyncEOF must have been created and registered with
 * the input NvMediaDLA handle NvMediaDlaRegisterNvSciSyncObj().
 *
 * @usage
 * - Allowed context for the API call
 *   - Interrupt handler: No
 ...
 *
 * @pre
 *  - The NvSciSyncObj associated with @a prenvscisyncfence must
 * have been registered with the input NvMediaDLA handle using
 * NvMediaDlaRegisterNvSciSyncObj().
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * If you use %NvMediaDlaGetSOFNvSciSyncFence(), you must call it after
 * calling the DLA submit function.
 *
 * \param[in] dla                An NvMedia DLA device handle.
 *                               @inputrange A non-null pointer to an
 *                               \ref NvMediaDla created with NvMediaDlaCreate().
 ...
 *
 * @pre
 *  - @a sofnvscisyncobj must have been created and registered with
 * the input NvMediaDLA handle using %NvMediaDlaRegisterNvSciSyncObj()
 *  - @a sofnvscisyncobj must have been set as the active SOF NvSciSyncObj using
 * NvMediaDlaSetNvSciSyncObjforSOF() for non-deterministic NvSciSyncObj and
 * NvMediaDlaInsertSOFNvSciSyncObj() for deterministic NvSciSyncObj
 ...
 *
 * @pre
 *  - @a eofnvscisyncobj must have been created and registered with
 * the input NvMediaDLA handle using NvMediaDlaRegisterNvSciSyncObj().
 *  - @a eofnvscisyncobj must have been set as the active EOF NvSciSyncObj using
 *  NvMediaDlaSetNvSciSyncObjforEOF() for non-deterministic NvSciSyncObj and
 *  NvMediaDlaInsertEOFNvSciSyncObj() for deterministic NvSciSyncObj
 *  - A task must have been submitted to the DLA engine using %NvMediaDlaSubmit().
 *
 * @usage
 * - Allowed context for the API call
 ...
 *
 * Covers: [NvMediaDLA_DES_37]
 *
 * If you use %NvMediaDlaInsertSOFNvSciSyncObj(),
 * - the application must call it for NvsciSyncObj backed by deterministic primitive only
 * - the application must call it before calling a DLA submit API &
 * before NvSciSyncObj gets signaled by DLA prior to the start of execution.
 *
 ...
 *                              NvSciSyncObj that is already registered as
 ...
 *
 * @pre
 *  - @a syncObj must have been created and registered with
 * the input NvMediaDLA handle using NvMediaDlaRegisterNvSciSyncObj().
 *
 *  @usage
 * - Allowed context for the API call
 ...
 *
 * Covers: [NvMediaDLA_DES_36]
 *
 *
 * If you use %NvMediaDlaInsertEOFNvSciSyncObj(),
 * - the application must call it for NvsciSyncObj backed by deterministic primitive only
 * - the application must call it before calling a DLA submit API and before
 * NvSciSyncObj gets signaled by DLA immediately after task execution completion.
 *
 * You can set a maximum of \ref <TODO> prefences by calling
