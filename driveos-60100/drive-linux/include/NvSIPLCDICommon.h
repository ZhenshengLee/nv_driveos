 ...
 *  object.
 * @note This structure is deprecated and will be removed in the next version.
 */
typedef struct {
...
typedef struct DevBlkCDISensorAttributes {
    /**
     * Holds the name as a string. If not supported, set to NULL.
     */
     ...
     * Valid Range:
     * @li Should be a valid @ref NVSIPL_PIXEL_ORDER
     * @li Refer to INvSIPLCamera::RegisterImages documentation for supported component orders
     */
    uint32_t  sensorCFA;
    ...
    /**
     * @brief Holds the sensor exposure ranges (in seconds) for active exposures.
     *
     ...
     * Valid Range: [1 , @ref DEVBLK_CDI_MAX_FRAME_REPORT_BYTES]
     */
 ...
 * tracked separately. When @ref DevBlkCDIExposure is embedded in another
 * structure, it is known according to the following members:
 *
 *   - @ref DevBlkCDIEmbeddedDataInfo::numExposures
 *   - @ref DevBlkCDISensorControl: The number of exposures supported by the
...
typedef struct {
     ...
     * Supports exposures times for up to @ref DEVBLK_CDI_MAX_EXPOSURES
     ...
     * @ref DevBlkCDIEmbeddedDataInfo::numExposures elements.
     * Supports gains for up to @ref DEVBLK_CDI_MAX_EXPOSURES
     * Valid Range: [ 0.0, 1000.0].
     */
...
/**
 * @brief Per-channel gains, intended for use with @ref DevBlkCDIWhiteBalance.
 */
typedef struct {
    /**
     ...
     *
     * Supports color components @ref DEVBLK_CDI_MAX_COLOR_COMPONENT
     * Valid Range: [1.0, 1000.0].
     */
 ...
 *
 * To use this structure correctly, the number of valid exposures must be
 * tracked separately. When accessed via a @ref DevBlkCDIEmbeddedDataInfo structure,
 * this is known by virtue of @ref DevBlkCDIEmbeddedDataInfo::numExposures.
 */
typedef struct {
    /**
...
typedef struct {
     ...
     * Valid Range: [1, @ref DEVBLK_CDI_MAX_NUM_TEMPERATURES].
     */
    uint8_t numTemperatures;

    /**
     * Holds the values of active sensor temperatures in degrees Celsius.
     * Array indexes must be in the range [0, (@a numTemperatures-1)].
     *
     */
