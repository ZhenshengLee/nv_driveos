...
struct SensorInfo
{
    ...
    struct Resolution
    {
         ...
         * from @ref NVSIPL_CAP_MIN_IMAGE_WIDTH to @ref NVSIPL_CAP_MAX_IMAGE_WIDTH.
         * @anon_struct_member
         */
        uint32_t width {0U};

        /** @brief Holds the height in pixels in the range
         * from @ref NVSIPL_CAP_MIN_IMAGE_HEIGHT to @ref NVSIPL_CAP_MAX_IMAGE_HEIGHT.
         * @anon_struct_member
         */
        uint32_t height {0U};
    };
    ...
    struct VirtualChannelInfo
    {
         ...
         *  in the range from @ref NVSIPL_CAP_MIN_FRAME_RATE to @ref NVSIPL_CAP_MAX_FRAME_RATE.
         * @anon_struct_member{1}
         */
        float_t fps {0.0F};
     ...
     * Even though you set this - if sensor does not support it then it does not authenticate anything */
    bool isAuthEnabled {true};
#else
    // Linux doesn't support authentication
...
struct DeserInfo
{
    ...
    /** @brief Holds a flag which indicates whether CDI (Camera Device Interface)
     ...
     For Safety build, CDI is always using version 2 API */
    bool useCDIv2API {false};
    /** @brief Holds flag to indicate that the deser needs to run a reset all sequence at startup */
    bool resetAll {false};
};
...
struct DeviceBlockInfo
{
    ...
    /** @brief When the camera boot accelerator has been configured, begin
     * streaming on camera modules one-by-one in the order they appear in the
     * platform configuration.
     *
     * By default this behavior is disabled - all camera modules will begin
     * streaming at the same time.
     *
     * This flag will not affect the behavior of the @ref isGroupInitProg flag
     * when initialization is being performed.
     *
