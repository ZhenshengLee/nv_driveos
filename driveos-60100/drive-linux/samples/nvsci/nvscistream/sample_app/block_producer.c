/* NvSciStream Safety Sample App - producer block
 *
...
/*
 * This implements the producer: cuda to cuda streaming
 */
...
typedef struct {
    ...
    uint32_t                finished;
    /* Flag indicating producer is in the proxy application */
    bool                    isProxyApp;
} ProdData;
    ...
    ProdData*         prodData)
{
    ...
    /* Check if producer is in the proxy application */
    if (prodData->isProxyApp) {

        pthread_mutex_lock(&mutex);
        producerPktAvailable = true;
        pthread_cond_signal(&cond);

        /* Check for available producer packet for copying the
        * data from consumer packet.
        */
        if (!bufferCopyDone && !streamEnd) {
            pthread_cond_wait(&cond, &mutex);
        }

        /* Copy the data from consumer packet to producer packet */
        memcpy(packet->dataSrcMem, tempBuffer, prodData->dataSize);
...
void* handleProducer(void *args)
{
    ...
    ProdArgs *prodArgs = (ProdArgs*)args;

    prodData->common.block = prodArgs->block;
    prodData->isProxyApp = prodArgs->isProxyApp;
    ...
    while (true) {
        // Query for events
        err = NvSciStreamBlockEventQuery(producer, -1, &event);
        ...
        case NvSciStreamEventType_Error:
            err = NvSciStreamBlockErrorGet(producer, &status);
            if (NvSciError_Success != err) {
                printf("Producer Failed to query the error event code %x\n",
                       err);
            } else {
                printf("Producer received error event: %x\n",
                       status);
            }
