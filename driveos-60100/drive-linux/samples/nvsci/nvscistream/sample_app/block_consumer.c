/* NvSciStream Safety Sample App - consumer block
 *
...
/*
 * This implements the consumer: cuda to cuda streaming
 */
...
typedef struct {
    ...
    uint32_t                counter;
    /* Flag indicating consumer is in the proxy application */
    bool                    isProxyApp;
} ConsData;
    ...
    ConsData*              consData)
{
    ...
    for (uint32_t i=0U; i<2U; ++i) {
        ...
        if (ELEMENT_NAME_DATA == type) {
            ...
            /* Create a temporary buffer for use when the consumer is in
            * proxy application.
            */
            tempBuffer = (uint8_t*)malloc(consData->dataSize);
    ...
    ConsData*         consData)
{
    ...
    /* Check if consumer is in the proxy application */
    if (consData->isProxyApp) {
        /* Check for available producer packet for copying the
        * data from consumer packet.
        */
        pthread_mutex_lock(&mutex);
        if (!producerPktAvailable) {
            pthread_cond_wait(&cond, &mutex);
        }

        /* Copy the data from consumer packet to producer packet */
        memcpy(tempBuffer, packet->dataDstMem, consData->dataSize);
        producerPktAvailable = false;
        bufferCopyDone = true;
...
void* handleConsumer(void *args)
{
    ...
    ConsArgs *consArgs = (ConsArgs*)args;

    consData->common.block = consArgs->block;
    consData->isProxyApp = consArgs->isProxyApp;
    ...
    while(true) {
        err = NvSciStreamBlockEventQuery(consData->common.block, -1, &event);
