/* NvSciStream Safety Sample App - pool block
 *
...
void* handlePool(void *args)
{
    ...
    PoolArgs *poolArgs = (PoolArgs*)args;

    /* Save the c2c pool flag */
    poolData->isC2cPool = poolArgs->isC2cPool;

    /* Save the packet count */
    poolData->numPacket = poolArgs->numPacket;

    poolData->common.block = poolArgs->block;
    ...
    while (true) {
        err = NvSciStreamBlockEventQuery(poolData->common.block, -1, &event);
