...
static void help(void)
{
    ...
    printf("\n [-i]            Enable synchronous payload transfer at runtime.");
    printf("\n                 Only supported in inter-process use case.");
    printf("\n                 Default: Disable.");
    printf("\n [-d]            Disable IPC notification at runtime.");
    printf("\n                 Only supported in inter-process use case.");
    printf("\n                 Default: Enable.");
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "pP:c:C:n:k:f:b:s:r:t:a:m:u:idlvh")) != -1) {
        switch (opt) {
        case 'p':
        ...
        case 'u':
            for (uint32_t i = 0; i < sizeof(testArg.gpuUUID.bytes); i += 1) {
                ...
                sscanf(strOptArg + 2 * i,
                       "%02hx",
                       (uint16_t*)&testArg.gpuUUID.bytes[i]);
            }
            gpuUUIDSet = true;
            break;
        case 'i':
            testArg.syncSend = true;
            break;
        case 'd':
            testArg.ipcEventNotifyDisabled = true;
            break;
        default:
            help();
            return 0;
    ...
    // Disabling IPC notification and enabling synchronous send at runime
    //  is only supported in inter-process use case.
    if ((testArg.syncSend || testArg.ipcEventNotifyDisabled) &&
        (testArg.isC2c || (testArg.testType == SingleProc))){
            printf("ERR: Sync-send and disabling IPC event notification"
                   "are only supported in inter-process use case.\n");
            help();
            return -1;
        }
