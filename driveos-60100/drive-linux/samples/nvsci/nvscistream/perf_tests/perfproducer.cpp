    ...
    std::vector<NvSciIpcEndpoint>* ipcEndpoint)
{
    ...
    if (testArg.testType == CrossProcProd) {
        ipcEP.resize(testArg.numConsumers, 0U);

        for (uint32_t i{ 0U }; i < testArg.numConsumers; i++) {
            ipcEP[i] = (*ipcEndpoint)[i];

            if (testArg.isC2c) {
                CHECK_NVSCIERR(NvSciStreamFifoQueueCreate(&c2cQueue[i]));
            }

            CHECK_NVSCIERR(NvSciStreamIpcSrcCreate2(ipcEP[i],
                                                    syncModule,
                                                    bufModule,
                                                    c2cQueue[i],
                                                    &ipcSrc[i]));
            ...
            if (testArg.syncSend) {
                ...
                CHECK_NVSCIERR(NvSciStreamPacketSendSync(ipcSrc[i]));
            }
        }
    }
...
void PerfProducer::handleInternalEvents(void)
{
    // IpcSrc block processes the internal message from the pair ipcDst
    for (uint32_t i{ 0U }; i < testArg.numConsumers; i++) {
        NvSciError err = NvSciStreamBlockHandleInternalEvents(ipcSrc[i]);
...
void PerfProducer::handleSetupComplete(void)
{
    // Disable IPC event notification at runtime
    if (testArg.ipcEventNotifyDisabled) {
        for (uint32_t i{ 0U }; i < testArg.numConsumers; i++) {
            CHECK_NVSCIERR(NvSciIpcEnableNotification(ipcEP[i], false));
        }
        ipcNotifyOff = true;
    }
...
void PerfProducer::handlePayload(void)
{
    ...
    if (testArg.latency && (numPayloads >= numRecvPackets)) {
        releaseEndTime = getTimeStamp(testArg.ptpTime);
    }
    ...
    if (testArg.latency) {
        ...
        uint64_t presentStartTime { getTimeStamp(testArg.ptpTime) } ;
