...
typedef struct {
    ...
    uint32_t                counter;
    /* Index of dataSrcMem array */
    uint32_t                idxSrcMem;
    /* Flag indicating producer has finished generating all payloads */
    uint32_t                finished;
    /* Number of frames that producer is configured to send */
    uint32_t                framesDesired;
} ProdData;
    ...
    ProdData*         prodData)
{
    ...
    if (NvSciError_Success != sciErr) {
        if ((opts.numLateConsumer > 0U) && (sciErr == NvSciError_StreamNotConnected)) {
            printf("[WARN] Producer failed (%x) to obtain packet for payload\n", sciErr);
        } else {
    ...
    *((uint32_t*)(packet->crcPtr)) = generateCRC(packet->dataSrcMem,
    ...
    if (NvSciError_Success != sciErr) {
        if ((opts.numLateConsumer > 0U) && (sciErr == NvSciError_StreamNotConnected)) {
            printf("[WARN] Producer failed (%x) to set postfence\n", sciErr);
        } else {
    ...
    if (NvSciError_Success != sciErr) {
        if ((opts.numLateConsumer > 0U) && (sciErr == NvSciError_StreamNotConnected)) {
            printf("[WARN] Producer failed (%x) to present packet\n", sciErr);
        } else {
    ...
    if (prodData->counter == prodData->framesDesired) {
    ...
    uint32_t  wait)
{
    ...
    if ((rv < 0) || (1 < rv)) {
        if ((rv < 0) && (opts.numLateConsumer > 0U)) {
            printf("Producer disconnected after sending %d payloads\n", prodData->counter);
            rv = 2;
        }
        deleteProducer(prodData);
    ...
    uint32_t numFrames)
{
    ...
    prodData->framesDesired = numFrames;
