...
typedef struct {
    ...
    uint32_t                counter;
    /* Number of frames that the consumer is interested in receiving.
     * This is mainly used for late/re-attach usecase, where a consumer
     * can decide to receive certain number of frames before disconnecting.
    */
    uint32_t                frames;

    /* Flag indicating consumer has finished receiving desired number of payloads */
    bool                    finished;
} ConsData;
    ...
    ConsData*         consData)
{
    ...
    if (consData->frames == consData->counter) {
        consData->finished = 1;
    }
    return 1;
}
    ...
    uint32_t  wait)
{
    ...
    case NvSciStreamEventType_PacketReady:
        if (!handleConsumerPayload(consData)) {
            rv = -1;
        } else if (consData->finished) {
            printf("Consumer finished receiving %d payloads\n", consData->counter);
            rv = 2;
        }
        break;
    }
