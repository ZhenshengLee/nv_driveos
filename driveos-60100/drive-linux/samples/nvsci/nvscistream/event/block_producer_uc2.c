...
typedef struct {
    ...
    uint32_t                finished;
    /* Fence for recently submitted operation */
    ...
    ProdData* prodData)
{
    ...
    {
        deletePacket(prodData, &prodData->packets[i]);
    }

    /* Free the sync objects and fence */
    NvSciSyncFenceClear(&prodData->postfence);
    ...
    ProdData*     prodData)
{
    ...
    /* Get Nvmedia surface type for A8R8G8B8/Y8U8Y8V8 buffers */
    NvSciBufType bufType = NvSciBufType_Image;
    NvSciBufAttrValColorFmt colorFmt = opts.yuv ?
                                       NvSciColor_Y8U8Y8V8 :
                                       NvSciColor_A8R8G8B8;
    ...
    ProdData*         prodData)
{
    ...
    do {
        /* Prepare the compose parameter */
        NvMedia2DComposeParameters params;
        nvmErr = NvMedia2DGetComposeParameters(prodData->nvm2d, &params);
        if (NVMEDIA_STATUS_OK != nvmErr) {
        ...
        uint32_t index = 0;
        nvmErr = NvMedia2DSetSrcNvSciBufObj(prodData->nvm2d,
                                            params,
                                            index,
                                            packet->srcBuf);
        if (NVMEDIA_STATUS_OK != nvmErr) {
        ...
        nvmErr = NvMedia2DSetDstNvSciBufObj(prodData->nvm2d,
                                            params,
                                            packet->dataBuf);
        if (NVMEDIA_STATUS_OK != nvmErr) {
            printf("Producer failed (%x) to set source buf\n", nvmErr);
        ...
        /* Perform a 2D compose operation */
        nvmErr = NvMedia2DCompose(prodData->nvm2d, params, &result);
        if (NVMEDIA_STATUS_OK == nvmErr) {
            break;
        } else if (NVMEDIA_STATUS_TIMED_OUT == nvmErr) {
            /*
             * Note: There's restriction on the number of pending operations
             *   handled by the same NvMedia2D. See the NvMedia2DCompose API
             *   spec for more information.
             * The producer may hit the restriction when it issues commands
             *   for many payloads in advance. When NvMedia2DCompose returns
             *   timeout error, wait for the recently submitted operation
             *   complete and then try again.
             */
            sciErr = NvSciSyncFenceWait(&prodData->postfence,
                                        prodData->cpuWaitContext,
                                        0xFFFFFFFF);
            if (NvSciError_Success != sciErr) {
                printf("Producer failed (%x) to wait for the recent operation done\n",
                        sciErr);
                return 0;
            }
            NvSciSyncFenceClear(&prodData->postfence);
    ...
    } while (NVMEDIA_STATUS_TIMED_OUT == nvmErr);
